window.violations = function () {
  return wave.report.things._iconsByGroup
}
window.statistics = function () {
  return wave.results.statistics
}
window.hideAllIcons = function () {
  wave.report.things.hideAll()
}
const waveconfig = {
  debug: !1,
  extensionUrl: '',
...
var reportPageMessaging = {
  fn: {},
  init: function () {
    reportPageMessaging.fn.setupEventListeners(),
      reportPageMessaging.fn.setupAndDispatchEvent('getExtensionUrl', ''),
      reportPageMessaging.fn.checkThatConfigUrlEventHasFired()
  },
  extensionUrlTimeout: { limit: 1e4, current: 0 }
}
...
;(reportPageMessaging.fn.setupAndDispatchEvent = function (e, t) {
  const a = new CustomEvent(e, { detail: { data: t } })
  document.dispatchEvent(a)
}),
  (reportPageMessaging.fn.checkThatConfigUrlEventHasFired = function () {
    if (
      reportPageMessaging.extensionUrlTimeout.current >=
      reportPageMessaging.extensionUrlTimeout.limit
    )
      return (
        console.log('extension timed out: not able to get extensionUrl.'),
        void (reportPageMessaging.extensionUrlTimeout.current = 0)
      )
    waveconfig.extensionUrl == '' &&
      (waveconfig.debug && console.log('extensionUrl not yet set.'),
      setTimeout(reportPageMessaging.fn.checkThatConfigUrlEventHasFired, 25),
      (reportPageMessaging.extensionUrlTimeout.current += 25)),
      waveconfig.debug &&
        console.log('extensionUrl from messaging: ' + waveconfig.extensionUrl)
  }),
  (reportPageMessaging.fn.getMessageData = function (e) {
    let t = ''
    try {
      t = JSON.parse(e)
    } catch (a) {
  ...
  (reportPageMessaging.fn.setupEventListeners = function () {
    document.addEventListener('setExtensionUrl', function (e) {
      waveconfig.extensionUrl = reportPageMessaging.fn.getMessageData(e.detail)
    }),
      document.addEventListener('toggleStyles', function (e) {
        wave.tabsengine.toggleStyles(
          reportPageMessaging.fn.getMessageData(e.detail)
        )
      }),
      document.addEventListener('setCurrentTab', function (e) {
        wave.tabs.current = reportPageMessaging.fn.getMessageData(e.detail)
      }),
      document.addEventListener('sidebarLoaded', function (e) {
        waveconfig.debug && console.log('Report received: sidebarLoaded.')
      }),
      document.addEventListener('resultsReceived', function (e) {
        waveconfig.debug &&
          console.log(
            'Background confirms results recieved by sidebar. Start buildOutline'
          ),
          wave.tabsengine.buildOutline()
      }),
      document.addEventListener('contrastIconDetails', function (e) {
        if (wave.tabsengine.currentContrastIcon) {
          const t = reportPageMessaging.fn.getMessageData(
            e.detail
          ).foregroundColor
          const a = reportPageMessaging.fn.getMessageData(
            e.detail
          ).backgroundColor
          ;(wave.tabsengine.currentContrastIcon.target.style.opacity = 1),
            (wave.tabsengine.currentContrastIcon.target.style.color = '#' + t),
            (wave.tabsengine.currentContrastIcon.target.style.backgroundColor =
              '#' + a)
        }
      }),
      document.addEventListener('toggleGroup', function (e) {
        switch (reportPageMessaging.fn.getMessageData(e.detail).action) {
          case 'show':
            wave.report.things.showGroup(
              reportPageMessaging.fn.getMessageData(e.detail).group
            )
            break
          case 'hide':
            wave.report.things.hideGroup(
              reportPageMessaging.fn.getMessageData(e.detail).group
            )
        }
      }),
      document.addEventListener('toggleType', function (e) {
        switch (reportPageMessaging.fn.getMessageData(e.detail).action) {
          case 'show':
            wave.report.things.showType(
              reportPageMessaging.fn.getMessageData(e.detail).type
            )
            break
          case 'hide':
            wave.report.things.hideType(
              reportPageMessaging.fn.getMessageData(e.detail).type
            )
        }
      }),
      document.addEventListener('hoverHighlight', function (e) {
        ;(iconObject = wave.report.things.iconsByType(
          reportPageMessaging.fn.getMessageData(e.detail).iconName
        )[reportPageMessaging.fn.getMessageData(e.detail).index]),
          wave.code.hoverHighlight(
            iconObject.target,
            reportPageMessaging.fn.getMessageData(e.detail).highlight
          )
      }),
      document.addEventListener('iconClickAction', function (e) {
        const t = wave.report.things.iconsByType(
          reportPageMessaging.fn.getMessageData(e.detail).iconType
        )[reportPageMessaging.fn.getMessageData(e.detail).index]
        wave.code.iconClickAction(t)
      }),
      document.addEventListener('refreshPage', function () {
        location.reload()
      }),
      document.addEventListener('toggleSaturation', function (e) {
        wave.tabsengine.toggleSaturation(
          reportPageMessaging.fn.getMessageData(e.detail).action
        )
      }),
      document.addEventListener('showNavOrder', function (e) {
        wave.tabsengine.showNavOrder(
          reportPageMessaging.fn.getMessageData(e.detail).action,
          reportPageMessaging.fn.getMessageData(e.detail).ShowIcons
        )
      }),
      document.addEventListener('showWaveIcons', function (e) {
        wave.tabsengine.showWaveIcons(
          reportPageMessaging.fn.getMessageData(e.detail).action
        )
      }),
      document.addEventListener('scrollToNavIcon', function (e) {
        wave.tabsengine.scrollToNavIcon(
          reportPageMessaging.fn.getMessageData(e.detail).action
        )
      })
  }),
  reportPageMessaging.init(),
  ((wave = {
    version: '3.2.7.2',
    allTags: null,
    code: null,
  ...
  }).fn.assignWaveIds = function (e) {
    try {
      for (let t = 0; t < e.length; t++) e[t].setAttribute('data-waveid', t + 1)
    } catch (e) {
  ...
  (wave.fn.removeWaveIds = function () {
    try {
      for (let e = 0; e < wave.allTags.length; e++)
  ...
  (wave.fn.reset = function () {
    waveconfig.platform != 'extension' &&
      document.getElementById('report') &&
      document.getElementById('report').replaceChildren(),
      document.getElementById('wave5_iconbox') &&
        (document.getElementById('wave5_iconbox').style.display = 'none'),
      document.getElementById('wave5topbar') &&
        (document.getElementById('wave5topbar').replaceChildren(),
        (document.getElementById('wave5topbar').onclick = function (e) {
          wave.iconbox.hide()
        })),
      waveconfig.platform != 'extension' || waveconfig.isSidebar
        ? ((wave.code = new wave.Code()),
          wave.engine.fn.resetEngine(),
          (wave.tabsengine.codePanelHasRun = !1))
        : ((wave.code = new wave.Code()), wave.engine.fn.resetEngine()),
  ...
  (wave.fn.run = function () {
    return (
      ...
      wave.engine.run().then(function () {
        ;(wave.results.categories = wave.engine.results),
          (wave.results.statistics = wave.engine.statistics)
        wave.engine.ruleTimes
        return (
          (wave.results.statistics.pagetitle = wave.page.title),
          (wave.results.statistics.totalelements = wave.allTags.length),
          (wave.results.success = !0),
          wave.engine.fn.structureOutput(),
          wave.results.success
            ? ((wave.report.iconlist = wave.results.categories),
              (wave.report.title = wave.results.statistics.pagetitle),
              wave.engine.icons.setStats(wave.results.statistics))
            : wave.tabs.showError(
                'Could not get the report results. Please try your request again.'
              ),
          wave.fn.showThings(),
          waveconfig.platform == 'extension'
            ? reportPageMessaging.fn.setupAndDispatchEvent(
                'waveResults',
                wave.report.iconlist
              )
            : wave.tabs.updatePanels(wave.report.iconlist),
          wave.fn.checkacsb(),
          wave.tabsengine.codePanelOpen
            ? wave.fn.showCode()
            : (document.getElementById('wave5spinner').display = 'block'),
          (document.querySelector('#wave5codetoggle a').onclick = function (e) {
            e.preventDefault(), wave.fn.loadCodePanel(e)
          }),
          wave.results
        )
      })
  ...
  (wave.fn.setupTags = function () {
    try {
      ...
      throw (
        (wave.tabs.showError(
          "There is a scripting conflict with content on this page. We recommend using the WAVE <a href='https://wave.webaim.org/extension/'>Chrome or Firefox Extension</a> to evaluate this page."
        ),
        new Error('Scripting conflict with content on the page: ' + e))
      )
    }
  }),
  (wave.fn.checkacsb = function () {
    for (i = 0; i < document.getElementsByTagName('script').length; i++)
      document.getElementsByTagName('script')[i].src.search(/acsb/i) > 0 &&
        (waveconfig.platform != 'extension' || waveconfig.isSidebar
          ? wave.tabs.showacsbalert('accessiBe')
          : reportPageMessaging.fn.setupAndDispatchEvent(
              'showacsbalert',
              'accessiBe'
            ))
  ...
  (wave.fn.resizeReportWindow = function () {
    if (waveconfig.platform != 'extension') {
      const e =
        window.innerHeight -
        document.getElementById('wave5topbar').offsetHeight -
        document.getElementById('wave5bottombar').offsetHeight
      document.getElementById('report').style.height = e + 'px'
    }
  }),
  (wave.code = null),
  (wave.rules = {}),
  ...
  (wave.fn.initialize = function () {
    const e = waveconfig.extensionUrl.substring(
      0,
      waveconfig.extensionUrl.length - 1
    )
    ;(wave.conf = { paths: { site: e } }), wave.fn.newReport()
  }),
  (wave.fn.newReport = function () {
    wave.fn.reset()
    let e = {}
    return (
      waveconfig.isSidebar || ((wave.page = document), (e = wave.fn.run())), e
    )
  }),
  (wave.fn.showThings = function () {
    const e = wave.report.iconlist
    const t = wave.engine.icons.docs
    ;(wave.report.things = new wave.manager.ThingManager(e, t, wave.page, {
      xpath: !1
    })),
  ...
  (wave.fn.reportReadyEvent = function () {
    waveconfig.debug &&
      console.log('Start reportReadyEvent. Sidebar: ' + waveconfig.isSidebar),
      wave.fn.removeWaveIds(wave.allTags),
      waveconfig.isSidebar &&
        (wave.tabs.removeLoadingAnimation(), wave.tabs.doResize()),
      (wave.iconbox = new wave.manager.IconBox()),
      wave.fn.insertIconBoxForChromeExtension(),
      wave.report.things.updateTopbar(),
      waveconfig.debug && console.log('End reportReadyEvent')
  }),
  (wave.fn.insertIconBoxForChromeExtension = function () {
    if (!waveconfig.isSidebar) {
      const e = document.createElement('div')
      e.id = 'wave5_iconbox'
      const t = document.createElement('div')
      t.id = 'wave5_title'
      const a = document.createElement('div')
      a.id = 'wave5_iconbox_title'
      const n = document.createTextNode('Icon Title')
      a.appendChild(n)
      const i = document.createElement('div')
      i.id = 'wave5_details'
      const s = document.createElement('div')
      s.id = 'wave5_iconbox_summary'
      const o = document.createElement('div')
      o.id = 'wave5_iconbox_doclink'
      const r = document.createElement('div')
      ;(r.id = 'wave5_iconbox_arrow'),
        t.appendChild(a),
        e.appendChild(t),
        i.appendChild(s),
        i.appendChild(o),
  ...
  typeof trustedTypes === 'undefined' &&
    (trustedTypes = { createPolicy: (e, t) => t }),
  (wave.utilities.escapeHTMLPolicy = trustedTypes.createPolicy('forceInner', {
    createHTML: (e) => e
  })),
  ...
  (wave.manager = {}),
  (wave.manager.style = {}),
  (wave.manager.selectedIcon = null),
  ...
  (wave.manager.ThingManager = function (e, t, a) {
    for (const n in ((this.contextPage = a),
    ...
    (this.allTagsCached['#'] = document.getElementById('wave5topbar')),
    e))
      if (((wave.engine.icons.stats[n] = e[n].count), e[n].count > 0))
        for (const i in e[n].items)
          for (let s = e[n].items[i], o = 0; o < s.xpaths.length; o++) {
            const r = s.xpaths[o]
            if (void 0 !== (s.contrastdata && s.contrastdata[o]))
              var l = new wave.manager.Icon(
                s.id,
                t[s.id],
                r,
                this.allTagsCached[r],
                s.text[o],
                s.contrastdata[o]
              )
            else
              l = new wave.manager.Icon(
                s.id,
                t[s.id],
                r,
                this.allTagsCached[r],
            ...
            this.attach(l),
              this.iconList.push(l),
              void 0 === this.iconBaseXpaths[r] &&
                (this.iconBaseXpaths[r] = []),
              this.iconBaseXpaths[r].push(l),
              this._iconsByGroup[n] || (this._iconsByGroup[n] = []),
              this._iconsByGroup[n].push(l),
              this._typesByGroup[n] || (this._typesByGroup[n] = []),
              this._typesByGroup[n].indexOf(s.id) == -1 &&
                (this._typesByGroup[n].push(s.id),
                (this._iconsByType[s.id] = [])),
              this._iconsByType[s.id].push(l)
          }
  }),
  (wave.manager.ThingManager.prototype.attach = function (e) {
    ...
    switch (e.position) {
      case 'before':
        t = e.target.insertAdjacentElement('beforebegin', e.iconElement)
        break
      case 'after':
        t = e.target.insertAdjacentElement('afterend', e.iconElement)
        break
      case 'first':
        if (
          e.target.nodeName &&
          e.target.nodeName.search(/(INPUT|IMG|SVG|TEXTAREA|SELECT)$/i) !== -1
        ) {
          t = e.target.insertAdjacentElement('beforebegin', e.iconElement)
          break
        }
        t = e.target.insertAdjacentElement('afterbegin', e.iconElement)
        break
      case 'last':
        if (
          e.target.nodeName &&
        ...
        t = e.target.insertAdjacentElement('beforeend', e.iconElement)
        break
      default:
        throw 'Unknown insert position: ' + position
    }
    return e.hasText
      ? e.iconElement.insertAdjacentElement('afterend', e.textElement)
      : t
  }),
  ...
  (wave.manager.ThingManager.prototype.showAll = function () {
    for (let e = 0; e < this.iconList.length; e++) this.iconList[e].show()
    for (var t in this._iconsByGroup)
      (document.getElementById('#toggle_group_' + t).checked = !0),
        document.getElementById('#toggle_group_' + t).click()
    for (const a in this._iconsByType)
      (document.getElementById('#toggle_type_' + t).checked = !0),
        document.getElementById('#toggle_type_' + t).click()
    this.updateTopbar()
  }),
  (wave.manager.ThingManager.prototype.hideAll = function () {
    for (let e = 0; e < this.iconList.length; e++) this.iconList[e].hide()
    this.updateTopbar()
  }),
  (wave.manager.ThingManager.prototype.showType = function (e) {
    for (let t = this.iconsByType(e), a = 0; a < t.length; a++) t[a].show()
    this.updateTopbar()
  }),
  (wave.manager.ThingManager.prototype.hideType = function (e) {
    for (let t = this.iconsByType(e), a = 0; a < t.length; a++) t[a].hide()
    this.updateTopbar()
  }),
  (wave.manager.ThingManager.prototype.showGroup = function (e) {
    const t = this.iconsByGroup(e)
    if (t) {
      for (let a = 0; a < t.length; a++) t[a].show()
      this.updateTopbar()
    }
  }),
  (wave.manager.ThingManager.prototype.hideGroup = function (e) {
    const t = this.iconsByGroup(e)
    if (t) {
      for (let a = 0; a < t.length; a++) t[a].hide()
      this.updateTopbar()
    }
  }),
  (wave.manager.ThingManager.prototype.updateTopbar = function () {
    let e = !1
    const t = document.getElementById('wave5topbar')
    if (
      (t.querySelectorAll('img').forEach(function (t, a) {
        e == 0 &&
          t.style.display != 'none' &&
          t.style.visibility != 'hidden' &&
          t.getAttribute('hidden') != 'hidden' &&
          (e = !0)
      }),
      e)
    ) {
      if (
        ...
        document.getElementById('pagewide_txt') == null)
      ) {
        ;(pagewide_txt = document.createElement('div')),
          (pagewide_txt.id = 'pagewide_txt')
        const a = document.createTextNode(
          'The following apply to the entire page: '
        )
        pagewide_txt.appendChild(a), t.prepend(pagewide_txt)
      }
    } else t.style.display = 'none'
    wave.fn.resizeReportWindow()
  }),
  (wave.manager.Icon = function (e, t, a, n, i, s) {
    ;(this.type = e),
      (this.data = t),
      (this.itemXPath = a),
      (this.target = n),
      (this.hasText = !1 !== i && i.length != 0),
      (this.iconText = i),
      (this.iconContrastdata = s),
  ...
  (wave.manager.Icon.prototype.create = function (e) {
    ...
    if (
      ...
      t.setAttribute(
        'src',
        wave.conf.paths.site + '/img/icons/' + this.type + '.svg'
      ),
      t.setAttribute('class', 'wave5icon'),
      t.setAttribute('tabindex', '0'),
      t.setAttribute('role', 'button'),
      this.iconContrastdata &&
        (this.iconElement.contrastData = this.iconContrastdata),
      Object.assign(t.style, wave.manager.style.icon),
      wave.engine.icons.docs[this.type])
    ) {
      const a =
        wave.engine.icons.docs[this.type].category.toUpperCase() +
        ': ' +
        wave.engine.icons.docs[this.type].title
      t.setAttribute('alt', a)
    } else t.setAttribute('alt', '')
    if (this.hasText) {
      wave5text = document.createElement('span')
      const n = document.createTextNode('*' + this.iconText + '*')
      wave5text.appendChild(n),
        (wave5text.className = 'wave5text'),
        Object.assign(wave5text.style, wave.manager.style.text),
        (this.textElement = wave5text)
    }
  }),
  (wave.manager.Icon.prototype.hide = function () {
    try {
      ;(this.iconElement.style.display = 'none'),
        this.hasText && (this.textElement.style.display = 'none')
    } catch (e) {
  ...
  (wave.manager.Icon.prototype.show = function () {
    try {
      ;(this.iconElement.style.display = ''),
        this.hasText && (this.textElement.style.display = '')
    } catch (e) {
      console.log('Could not hide element: ' + e)
    }
  }),
  (wave.manager.Icon.prototype.scrollToIcon = function () {
    if (waveconfig.platform == 'extension')
      window.scrollTo(
        0,
        this.iconElement.getBoundingClientRect().top +
      ...
      document
        ...
        .contentWindow.document.documentElement.scrollTo(
          0,
          this.iconElement.getBoundingClientRect().top +
            document.getElementById('report').contentWindow.document
              .documentElement.scrollTop -
            110
        )
      var e = document.createElement('style')
      ;(e.textContent = '@keyframes blink { 50% { opacity:0 ; } }'),
        document
          .getElementById('report')
          .contentWindow.document.body.appendChild(e)
    }
    ;(this.iconElement.style.animationPlayState = 'running'),
      (this.iconElement.style.animation = 'blink .25s'),
      (this.iconElement.style.animationIterationCount = '3'),
      (this.iconElement.onanimationend = function (t) {
        ;(t.target.style.animationPlayState = 'paused'),
          (t.target.style.animation = ''),
          waveconfig.platform != 'extension' && e.remove()
      })
  }),
  (wave.manager.IconBox = function () {
    const e = this
    this.hide(),
      (wave.page.onscroll = function (t) {
        e.hide()
      }),
      (wave.page.onclick = function (t) {
        e.hide()
      }),
      (wave.page.onkeyup = function (t) {
        t.keyCode == 27 && e.hide()
      })
    for (let t = wave.report.things.icons(), a = 0; a < t.length; a++)
      t[a].iconElement.addEventListener(
        'focus',
        (function (e) {
          return function () {
        ...
        t[a].iconElement.addEventListener(
          ...
          (function (e) {
            return function (t) {
              iconClick(e), t.preventDefault(), t.stopPropagation()
            }
          })(t[a])
        ),
        t[a].iconElement.addEventListener(
          ...
          (function (e) {
            return function (t) {
              ;(t.keyCode != 13 && t.keyCode != 32) || iconClick(e)
            }
          })(t[a])
        ),
        (iconClick = function (e) {
          if (
            (wave.tabsengine.codePanelOpen == 1 &&
              e.itemXPath !== '#' &&
              wave.code.setSelected(e),
            e.data.name == 'contrast' || e.type == 'contrast')
          )
            if (
              ((wave.tabsengine.currentContrastIcon = e),
              waveconfig.platform == 'extension')
            ) {
              const t = e.iconContrastdata
              const a = (t[0], t[1].substring(1))
              const n = t[2].substring(1)
              const i = t[3]
              reportPageMessaging.fn.setupAndDispatchEvent(
                'setSidebarContrastDetails',
                { fgcolor: a, bgcolor: n, largetext: i }
              )
            } else showContrasts(e.iconContrastdata)
          wave.tabs.current == 'tab-reference' &&
            (waveconfig.platform == 'extension'
              ? reportPageMessaging.fn.setupAndDispatchEvent(
                  'moreInfo',
                  e.data.name
                )
              : wave.tabs.moreInfo(e.type)),
            e.iconContrastdata[0] != null
              ? (document.getElementById('wave5_iconbox_title').innerHTML =
                  wave.utilities.escapeHTMLPolicy.createHTML(
                    wave.engine.icons.docs[e.type].title +
                      ' (' +
                      e.iconContrastdata[0] +
                      ':1)'
                  ))
              : (document.getElementById('wave5_iconbox_title').innerHTML =
                  wave.utilities.escapeHTMLPolicy.createHTML(
            ...
            (document.getElementById('wave5_iconbox_summary').innerHTML =
              wave.utilities.escapeHTMLPolicy.createHTML(
                wave.engine.icons.docs[e.type].summary
              ))
          const s = document.getElementById('wave5_iconbox_doclink')
          s.textContent = ''
          const o = document.createElement('a')
          o.href = '#'
          let r
          let l = document.createTextNode('Reference')
          o.appendChild(l),
            s.append(o),
            o.addEventListener(
              'click',
              ...
              function (e) {
                waveconfig.platform == 'extension'
                  ? reportPageMessaging.fn.setupAndDispatchEvent('moreInfo', r)
                  : wave.tabs.moreInfo(r),
                  e.preventDefault(),
                  e.stopPropagation()
              })
            )
          const c = document.createElement('a')
          ;(c.href = '#'), (c.className = 'viewInCodePanelLink')
          l = document.createTextNode('Code')
          c.appendChild(l),
            s.append(c),
            c.addEventListener('click', function (t) {
              wave.tabsengine.codePanelOpen || wave.fn.loadCodePanel(t),
                wave.code.setSelected(e),
                t.preventDefault(),
                t.stopPropagation()
            })
          const d = document.getElementById('wave5_iconbox')
          ...
          const p = wave.page.body.offsetWidth
          u =
            e.target.id == 'wave5topbar' || waveconfig.platform == 'extension'
              ? 0
              : document.getElementById('sidebar_container').offsetWidth
          const h = e.iconElement.getBoundingClientRect().left
          const b = e.iconElement.getBoundingClientRect().top
          const v =
            waveconfig.platform == 'extension' || e.target.id == 'wave5topbar'
              ? 0
              : document.getElementById('wave5topbar').getBoundingClientRect()
                  .height
          if (b < g + 18)
            var f = b + v + 52,
              w = 't'
          ...
          return (
            ...
            (d.style.left = y + 'px'),
            document
              .getElementById('wave5_iconbox_arrow')
              .removeAttribute('class'),
            document.getElementById('wave5_iconbox_arrow').classList.add(w + k),
            !1
          )
        })
  }),
  (wave.manager.IconBox.prototype.hide = function () {
    ;(iconbox = document.getElementById('wave5_iconbox')),
      iconbox &&
        iconbox.style.opacity == '1' &&
        (wave.code, (iconbox.style.display = 'none'))
  }),
  Array.prototype.indexOf ||
    (Array.prototype.indexOf = function (e) {
      ...
      if (
        ...
        return -1
      for (let i = n >= 0 ? n : Math.max(a - Math.abs(n), 0); i < a; i++)
...
var FrmInj = {
  ...
  func: {},
  init: function () {
    FrmInj.func.createTopBar(),
      FrmInj.func.createBottomBar(),
      FrmInj.func.createSidebar()
  }
}
if (
  ((FrmInj.func.createSidebar = function () {
    let e = getComputedStyle(FrmInj.vars.html).marginLeft
    ;(e =
      e === 'auto'
        ? 0
        : parseFloat(getComputedStyle(FrmInj.vars.html).marginLeft)),
      (FrmInj.vars.html.style.marginLeft =
        e + parseFloat(FrmInj.vars.sidebarWidth) + 'px')
    const t = document.createElement('iframe')
    ;(t.id = 'wave_sidebar_container'),
      (t.src = waveconfig.extensionUrl),
      (t.style.width = FrmInj.vars.sidebarWidth),
      (t.style.height = '100%'),
      (t.style.float = 'left'),
      (t.style.scrolling = 'yes'),
      (t.style.frameBorder = '0'),
      (t.style.allowtransparency = 'false'),
  ...
  (FrmInj.func.createTopBar = function () {
    const e = document.createElement('div')
    ;(e.id = 'wave5topbar'),
      (e.style.width = '100%'),
      document.body.insertBefore(e, document.body.firstChild)
  }),
  (FrmInj.func.createBottomBar = function () {
    let e = getComputedStyle(FrmInj.vars.html).bottom
    ;(e =
      e === 'auto' ? 0 : parseFloat(getComputedStyle(FrmInj.vars.html).bottom)),
      (FrmInj.vars.html.style.bottom =
        e + parseFloat(FrmInj.vars.bottombarHeight) + 'px')
    const t = document.createElement('div')
    ;(t.id = 'wave5bottombar'),
      t.setAttribute('role', 'region'),
      t.setAttribute('aria-label', 'Code panel'),
      (t.style.float = 'left'),
      (t.style.scrolling = 'no'),
      (t.style.frameBorder = '0'),
      (t.style.allowtransparency = 'false'),
      (t.style.position = 'fixed'),
      (t.style.border = 'none'),
    ...
    ;(a.id = 'wave5spinner'),
      ...
      (a.style.width = 'calc(100% - 380px)'),
      (a.style.display = 'block')
    let n = document.createTextNode('Loading...')
    a.appendChild(n)
    const i = document.createElement('div')
    i.id = 'wave5codetoggle'
    const s = document.createElement('a')
    ;(s.id = 'wave5codetogglelink'),
      (s.href = '#'),
      (s.style.background = 'transparent')
    const o = document.createElement('span')
    o.className = 'wavehidden'
    n = document.createTextNode('code')
    o.appendChild(n), s.appendChild(o), i.appendChild(s)
    const r = document.createElement('div')
    ;(r.id = 'wave5code'),
      t.appendChild(a),
      t.appendChild(i),
      t.appendChild(r),
      (t.style.left = FrmInj.vars.sidebarWidth),
  ...
  (wave.xpath = { mappedElementsByXPath: {}, XPath: {} }),
  ...
  (wave.xpath.convertIconListToXPathOutput = function (e) {
    const t = {}
    for (var a in e)
      for (const n in (t[a] || (t[a] = {}), e[a]))
        n != '0' && (t[a][this.XPath[n]] = e[a][n])
    return (t['#'] = e[a][0] ? e[a][0] : []), t
  }),
  (wave.xpath.getXPathByElement = function (e) {
    if (!e) return null
    for (var t = ''; e && e.nodeType == 1; e = e.parentNode)
      if (e.parentNode.nodeType == 1) {
        const a = [].filter.call(e.parentNode.children, function (t) {
          if (t.className != 'wave5icon') return t.tagName == e.tagName
        })
        ;(id = '[' + ([].indexOf.call(a, e) + 1) + ']'),
          e.tagName != 'HTML' && (t = '/' + e.tagName.toUpperCase() + id + t)
      }
    return '/HTML' + t
  }),
  (wave.xpath.calculateXPath = function (e) {
    if (!e) return '/'
    if (e.getAttribute('data-waveid') == '0') return '#'
    ;(this.XPath[e.getAttribute('data-waveid')] = '/'),
      (this.mappedElementsByXPath['/'] = e)
    for (let t = {}, a = 0; e.children && a < e.children.length; a++)
      t[e.children[a].nodeName] == null && (t[e.children[a].nodeName] = 0),
        t[e.children[a].nodeName]++,
        wave.xpath.calculateXPathPrivate(
          e.children[a],
          '/HTML/' +
            e.children[a].nodeName +
            '[' +
            t[e.children[a].nodeName] +
            ']'
        )
  ...
  (wave.xpath.calculateXPathPrivate = function (e, t) {
    if (e) {
      ...
      for (let a = {}, n = 0; e.childNodes && n < e.childNodes.length; n++)
        e.childNodes[n].nodeType == 1 &&
          (a[e.childNodes[n].nodeName] == null &&
            (a[e.childNodes[n].nodeName] = 0),
          a[e.childNodes[n].nodeName]++,
          wave.xpath.calculateXPathPrivate(
            e.childNodes[n],
            t +
              '/' +
              e.childNodes[n].nodeName +
              '[' +
              a[e.childNodes[n].nodeName] +
              ']'
          ))
  ...
  (wave.Code = function () {
    const e = document.getElementById('wave5code')
    e && e.remove(),
      (this.xpaths = {}),
      (this.htmlRoot = null),
  ...
  (wave.Code.prototype.setSelected = function (e) {
    ;(wave.manager.selectedIcon = e),
      this.selectedXpath &&
        document.querySelector(
          '[data-reportxpath="' + this.selectedXpath + '"]'
        ) != null &&
        document
          .querySelector('[data-reportxpath="' + this.selectedXpath + '"]')
          .classList.remove('selectedcode'),
      document.querySelector('[data-reportxpath="' + e.itemXPath + '"]') !=
        null &&
        document
          .querySelector('[data-reportxpath="' + e.itemXPath + '"]')
          .classList.add('selectedcode'),
      this.unhighlight()
    const t = document.querySelector('[data-reportxpath="' + e.itemXPath + '"]')
    t && (document.getElementById('wave5code').scrollTop = t.offsetTop - 50),
      (this.selectedElement = e.target),
      (this.selectedXpath = e.itemXPath),
      this.highlight()
  }),
  (wave.Code.prototype.displayTree = function () {
    const e = document.getElementById('wave5bottombar')
    try {
      e.append(this.displayTreePrivate(this.htmlRoot, ''))
    } catch (e) {
  ...
  (wave.Code.prototype.highlight = function () {
    this.selectedElement != null &&
      (this.selectedElement.style.outline = '2px dashed red')
  }),
  (wave.Code.prototype.unhighlight = function () {
    this.selectedElement != null && (this.selectedElement.style.outline = '')
  }),
  (wave.Code.prototype.hoverHighlight = function (e, t) {
    return (
      (this.selectedElement = e), t ? this.highlight() : this.unhighlight(), !1
    )
  }),
  (wave.Code.prototype.iconCodeImage = function (e, t) {
    const a = e.type
    const n = e.data.title
    const i = document.createElement('img')
    return (
      i.setAttribute('src', wave.conf.paths.site + '/img/icons/' + a + '.svg'),
      i.setAttribute('alt', n),
      (i.style.cursor = 'cursor:pointer'),
      e.hidden[t] &&
        (e.classList.add('wave5_hiddenicon'),
        i.setAttribute(
          'title',
          'This icon or the element it applies to is visually hidden in your page. Disable "Styles" to view it.'
        )),
      ...
      (i.onclick = function (t) {
        if (
          (wave.code.iconClickAction(e),
          waveconfig.platform == 'extension' && e.data.cat_code === 'contrast')
        ) {
          wave.tabsengine.currentContrastIcon = e
          const a = e.iconContrastdata
          const n = (a[0], a[1])
          const i = a[2]
          const s = a[3]
          reportPageMessaging.fn.setupAndDispatchEvent(
            'setSidebarContrastDetails',
            { fgcolor: n, bgcolor: i, largetext: s }
          )
        } else
          e.data.cat_code === 'contrast' && showContrasts(e.iconContrastdata)
      }),
      i
    )
  }),
  (wave.Code.prototype.iconClickAction = function (e) {
    e.scrollToIcon(),
      wave.iconbox.hide(),
      wave.tabsengine.codePanelOpen == 1 && wave.code.setSelected(e),
      e.data.cat_code === 'contrast' &&
        (wave.tabsengine.currentContrastIcon = e)
  }),
  (wave.Code.prototype.displayTreePrivate = function (e, t) {
    t == '' &&
      (t = document.createElement('div')).setAttribute('id', 'wave5code')
    if (
      e &&
      (e.classList == null ||
        !(
          e.classList.contains('wave5icon') ||
          e.classList.contains('wave5text') ||
          e.classList.contains('waveorder') ||
          e.getAttribute('id') == 'waveordercontainer' ||
          e.getAttribute('id') == 'wave5bottombar' ||
          e.getAttribute('id') == 'wave5_iconbox' ||
          e.getAttribute('id') == 'wave5topbar' ||
          e.getAttribute('id') == 'wave_sidebar_container' ||
          e.getAttribute('id') == 'wavescript' ||
          e.getAttribute('id') == 'wavestyle'
        ))
    ) {
      if (e.nodeType === 1 || e.nodeType === 9 || e.nodeType === 10) {
        const a = wave.xpath.getXPathByElement(e)
        if (void 0 !== wave.report.things.iconBaseXpaths[a]) {
          var n = document.createElement('div')
          n.setAttribute('data-reportxpath', a)
          for (
            let i = 0;
            i < wave.report.things.iconBaseXpaths[a].length;
            i++
          ) {
            const s = wave.report.things.iconBaseXpaths[a][i]
            void 0 !== s.data &&
              ((newCodeIcon = wave.code.iconCodeImage(s, 0)),
              n.prepend(newCodeIcon))
          }
        }
        const o = document.createElement('span')
        o.classList.add('wave5element'),
          (wave5open = document.createTextNode(
            '<' + htmlEntities(e.nodeName.toLowerCase())
          )),
          o.append(wave5open)
        for (var r = 0; e.attributes && r < e.attributes.length; r++)
          if (e.attributes[r].nodeName.indexOf('data-waveid') == -1) {
            const l =
              e.attributes[r].nodeName == 'wave5style'
                ? 'style'
                : e.attributes[r].nodeName
            if ((l == 'style' && e.attributes[r].value != '') || l != 'style') {
              ;(attributes = document.createElement('span')),
                attributes.classList.add('wave5attribute'),
                (attributes.textContent = ' ' + l + '="')
              const c = document.createElement('span')
              c.classList.add('wave5value'),
                (c.textContent = htmlEntities(e.attributes[r].value)),
                attributes.append(c),
                attributes.append('"'),
                o.append(attributes)
            }
          }
        ;(wave5close = document.createTextNode('>')), o.append(wave5close)
        for (r = 0; r < e.childNodes.length; r++)
          this.displayTreePrivate(e.childNodes[r], o)
        return (
          (e.innerHTML && e.innerHTML.length > 0) ||
          [
            'area',
            'base',
            'br',
            ...
            ? ((wave5text = document.createTextNode(
                '</' + htmlEntities(e.nodeName.toLowerCase()) + '>'
              )),
              o.append(wave5text),
              n ? (n.append(o), t.append(n)) : t.append(o))
            : n
              ? (n.append(o), t.append(n))
              : t.append(o),
          t
        )
      }
      const d = e.nodeValue.trim()
      if (e.nodeType === 8)
        (u = document.createElement('div')).classList.add('wave5comment'),
          (u.textContent = '\x3c!-- ' + htmlEntities(d) + '--\x3e'),
          t.append(u)
      else if (d.length > 0) {
        var u
        ;(u = document.createElement('span')).classList.add('wave5codetext'),
          (u.textContent = htmlEntities(d))
        const g = document.createElement('br')
        const m = document.createElement('br')
        t.append(g), t.append(u), t.append(m)
      }
    }
  }),
  (wave.engine = {
  ...
  (wave.engine.fn.setupResultStack = function () {
    for (const e in wave.engine.icons.docs) {
      const t = wave.engine.icons.docs[e].cat_code
      void 0 === wave.engine.results[t] &&
        (wave.engine.results[t] = {
  ...
  (wave.engine.fn.resetEngine = function () {
    ;(wave.engine.icons = {
      ...
      (wave.engine.statistics = { allitemcount: 0, totalelements: 0 })
  }),
  (wave.engine.run = async function (e) {
    const t = new Date()
    let a = null
    let n = null
    const i = wave.rules
    ...
    for (e in i) {
      ...
      ;(n = new Date()),
        (wave.engine.ruleTimes[e] = n - a),
        waveconfig.debug &&
          console.log('RULE: ' + e + ' (' + wave.engine.ruleTimes[e] + 'ms)')
    }
    const o = new Date()
    return (
      waveconfig.debug && console.log('TOTAL RULE TIME: ' + (o - t) + 'ms'),
      Promise.all([])
    )
  }),
  (wave.engine.fn.override = function (e, t) {
    wave.engine.overrides[e] || (wave.engine.overrides[e] = {})
    for (let a = 0; a < t.length; a++) wave.engine.overrides[e][t[a]] = !0
  }),
  (wave.engine.fn.overrideby = function (e, t) {
    wave.engine.overriddenby[e] || (wave.engine.overriddenby[e] = {})
    for (let a = 0; a < t.length; a++) wave.engine.overriddenby[e][t[a]] = !0
  }),
  (wave.engine.fn.hasTextContent = function (e) {
    ...
    return (
      (n.length > 0 && n.search(/^\s+$/i) == -1) ||
      (i && i.search(/^\s+$/i) == -1) ||
      (s && s.search(/^\s+$/i) == -1)
        ? (t = !0)
        : e
            .querySelectorAll(
              'img, svg, [role=button][aria-label], [role=link][aria-label], [role=img][aria-label]'
            )
            .forEach(function (n, i) {
              if (
                ...
                s && s.search(/^\s+$/i) == -1 && (t = !0)
                const o = n.getAttribute('title')
                o && o.search(/^\s+$/i) == -1 && (t = !0)
                const r = n.getAttribute('aria-label')
                r && r.search(/^\s+$/i) == -1 && (t = !0)
                const l = n.getAttribute('aria-labelledby')
                l && l.search(/^\s+$/i) == -1 && (t = !0)
              }
              a == 0 &&
              getComputedStyle(e).getPropertyValue('visibility') !== 'hidden' &&
              getComputedStyle(n).getPropertyValue('visibility') == 'hidden'
                ? (t = !1)
                : (a = !0)
            }),
      t
  ...
  (wave.engine.fn.isElementHidden = function (e) {
    if (
      ...
      return !0
    for (; e && e.parentNode && e.parentNode !== document; ) {
      if (
        e.hidden == 1 ||
        getComputedStyle(e).getPropertyValue('display') == 'none'
      )
  ...
  (wave.engine.fn.isLabeledByTag = function (e, t) {
    ...
    const s = t.querySelectorAll('label[for="' + n + '"]')
    if (s.length > 0)
      (a = !0),
        s.forEach(function (t, n) {
          wave.engine.fn.isElementHidden(t) == 1 &&
            !0 !== wave.engine.fn.isElementHidden(e) &&
            (a = !1)
        })
    else if (i !== null) {
      i.querySelectorAll(
        'input:not([type=image],[type=submit],[type=reset],[type=button],[type=hidden]), select, textarea, meter, output, progress'
      ).length == 1 &&
        (i.getAttribute('for')
          ? i.getAttribute('for') == n && (a = !0)
          : (a = !0))
    }
    return a
  }),
  (wave.engine.fn.isLabeled = function (e, t) {
    return (
      ...
      wave.engine.fn.isLabeledByTag(e, t)
    )
  }),
  (wave.engine.fn.previousElementSibling = function (e) {
    if (e.previousElementSibling !== 'undefined')
      return e.previousElementSibling
    for (; (e = e.previousSibling); ) if (e.nodeType === 1) return e
  }),
  (wave.engine.fn.getPath = function (e) {
    if (wave.reporttype != 4) return !1
    ...
    for (var t = []; e.nodeType === Node.ELEMENT_NODE; ) {
      ...
      if (e.id) a += '#' + e.id
        ...
        for (
          ...
          i.unshift(n.nodeName), (n = wave.engine.fn.previousElementSibling(n))
        i[0] !== 'HTML' && (i[0] = i[0] + ':first-child'), (a = i.join(' + '))
      }
      t.unshift(a), (e = e.parentNode)
    }
    return t.join(' > ')
  }),
  (wave.engine.fn.addIcon = function (e, t, a) {
    a || (a = !1)
    const n = e == '#' ? 0 : e.getAttribute('data-waveid')
    const i = wave.engine.icons.docs[t]
    const s = n === 0 ? '#' : wave.xpath.XPath[n]
    const o = wave.engine.fn.getPath(e)
    if (i) {
      if (wave.engine.overrides[t]) {
        for (var r in wave.engine.overrides[t])
          if (
            void 0 !== wave.engine.iconList[r] &&
            void 0 !== wave.engine.iconList[r].items[s]
          ) {
            delete wave.engine.iconList[r].items[s]
            break
          }
      } else if (wave.engine.overriddenby[t])
        for (var r in wave.engine.overriddenby[t])
          if (
            void 0 !== wave.engine.iconList[r] &&
            void 0 !== wave.engine.iconList[r].items[s]
          )
            return
      if (
        (wave.engine.iconList[t] || (wave.engine.iconList[t] = { items: {} }),
        wave.engine.iconList[t].items[s] == null)
      ) {
        const l =
          s !== '#' &&
          (e.offsetWidth == 0 ||
            ...
            e.offsetTop < 0 ||
            e.offsetLeft < 0)
        wave.engine.iconList[t].items[s] =
          t == 'contrast'
            ? {
                id: t,
                description: i.title,
            ...
            : { id: t, description: i.title, text: a, selector: o, hidden: l }
      } else wave.engine.iconList[t].items[s].text = a
    }
  }),
  (wave.engine.fn.addPagewideIcon = function (e, t) {
    this.addIcon('#', e, t)
  }),
  (wave.engine.fn.structureOutput = function () {
    for (const e in wave.engine.iconList) {
      const t = wave.engine.icons.docs[e]
      for (const a in wave.engine.iconList[e].items) {
        const n = t.cat_code
        wave.engine.statistics.allitemcount ||
          (wave.engine.statistics.allitemcount = 0),
          wave.engine.statistics.allitemcount++,
          wave.engine.results[n].count || (wave.engine.results[n].count = 0),
          wave.engine.results[n].count++,
          wave.engine.results[n].items[e]
            ? (wave.engine.results[n].items[e].count++,
              wave.engine.results[n].items[e].xpaths.push(a),
              wave.engine.results[n].items[e].selectors.push(
                wave.engine.iconList[e].items[a].selector
              ),
              wave.engine.results[n].items[e].text.push(
                wave.engine.iconList[e].items[a].text
              ),
              wave.engine.results[n].items[e].hidden.push(
                wave.engine.iconList[e].items[a].hidden
              ),
              wave.engine.results[n].items[e].id === 'contrast' &&
                wave.engine.results[n].items[e].contrastdata.push(
                  wave.engine.iconList[e].items[a].contrastdata
                ))
            : ((wave.engine.results[n].items[e] = {}),
              (wave.engine.results[n].items[e].id = e),
              (wave.engine.results[n].items[e].description = t.title),
              (wave.engine.results[n].items[e].count = 1),
              (wave.engine.results[n].items[e].xpaths = [a]),
              [wave.engine.iconList[e].items[a].selector] &&
                (wave.engine.results[n].items[e].selectors = [
                  wave.engine.iconList[e].items[a].selector
                ]),
              [wave.engine.iconList[e].items[a].text] &&
                (wave.engine.results[n].items[e].text = [
                  wave.engine.iconList[e].items[a].text
                ]),
              [wave.engine.iconList[e].items[a].hidden] &&
                (wave.engine.results[n].items[e].hidden = [
                  wave.engine.iconList[e].items[a].hidden
                ]),
              wave.engine.results[n].items[e].id === 'contrast' &&
                (wave.engine.results[n].items[e].contrastdata = [
                  wave.engine.iconList[e].items[a].contrastdata
                ]))
      }
    }
  ...
  (wave.engine.fn.elementHasText = function (e) {
    let t = !1
    if (!e) return !1
    for (let a = e.childNodes, n = 0; n < a.length; n++)
      if (
        a[n].nodeType == 3 &&
        a[n].nodeValue.length > 0 &&
        a[n].nodeValue.search(/^\s+$/i) == -1
      ) {
        t = !0
        break
      }
    return (
      e.tagName == 'INPUT' &&
        ...
        (t = !0),
      e.tagName == 'SELECT' && e.options.length > 0 && (t = !0),
      t
    )
  }),
  (wave.engine.fn.calculateContrast = function (e, t, a) {
    if (!e) {
      e = wave.page.body
      t = this.getColor(e, 'fg')
      if ((a = this.getColor(e, 'bg'))[3] == '0')
        (a = this.getColor(wave.page.querySelector('html'), 'bg'))[3] == '0' &&
          (a = ['255', '255', '255', '1'])
      wave.engine.fn.elementHasText(e) && this.checkContrast(e, t, a)
    }
    for (let n = 0; e.children && n < e.children.length; n++) {
      const i = e.children[n]
      if (
        i.getAttribute('id') !== 'wave5bottombar' &&
        i.tagName.toUpperCase() !== 'SVG' &&
        i.tagName !== 'SCRIPT' &&
        i.tagName !== 'IMG' &&
        i.tagName !== 'STYLE' &&
        i.tagName.toUpperCase() !== 'NOSCRIPT' &&
        i.tagName.toUpperCase() !== 'VIDEO' &&
        i.tagName.toUpperCase() !== 'AUDIO' &&
        i.tagName.toUpperCase() !== 'OPTION' &&
        i.disabled != 1 &&
        !0 !== i.hidden &&
        getComputedStyle(i).getPropertyValue('display') !== 'none' &&
        getComputedStyle(i).getPropertyValue('visibility') !== 'hidden' &&
        getComputedStyle(i).getPropertyValue('font-size') !== '0px' &&
        i.ariaDisabled !== 'true'
      )
        if (wave.engine.contrast[i.getAttribute('data-waveid')])
          i.childNodes && wave.engine.fn.calculateContrast(i, t, a)
        else {
          const s = this.getColor(i, 'fg')
          let o = this.getColor(i, 'bg')
          o[3] == '0' && (o = a),
            wave.engine.fn.elementHasText(i)
              ? s !== t || o !== a
                ? (this.checkContrast(i, s, o),
                  wave.engine.fn.calculateContrast(i, s, o))
                : wave.engine.fn.calculateContrast(i, s, o)
              : i.childNodes && wave.engine.fn.calculateContrast(i, s, o)
        }
    }
  }),
  (wave.engine.fn.getColor = function (e, t) {
    ...
    if (
      n.backgroundImage.indexOf('linear-gradient') > -1 ||
      (n.filter != 'none' && n.filter != 'blur(0px)')
    )
      return !1
    if (t == 'fg')
      (a = n.color.replace(/[^\d,.]/g, '').split(','))[3] || (a[3] = '1'),
        n.opacity && n.opacity < 1 && (a[3] = a[3] * n.opacity)
    else if (
      ((a = n.backgroundColor.replace(/[^\d,.]/g, '').split(','))[3] ||
        (a[3] = '1'),
      a[3] != 0 && Number(a[3]) < 1)
    )
      return !1
    if (a === 'transparent') a = [0, 0, 0, 0]
    else if (typeof a === 'string') {
      const i = a
      if (
        !(a = i.match(/rgba?\(([\d.]+), ([\d.]+), ([\d.]+)(?:, ([\d.]+))?\)/))
      )
        throw new Error('Invalid string: ' + i)
      a.shift()
    }
  ...
  (wave.engine.fn.checkContrast = function (e, t, a) {
    if (!1 !== t && !1 !== a) {
      const n = t[3]
      n < 1
        ? ((fgcolor[0] = (1 - n) * a[0] + n * t[0]),
          (fgcolor[1] = (1 - n) * a[1] + n * t[1]),
          (fgcolor[2] = (1 - n) * a[2] + n * t[2]))
        : (fgcolor = t)
      const i = wave.engine.fn.getLuminance(fgcolor) + 0.05
      const s = wave.engine.fn.getLuminance(a) + 0.05
      let o = i / s
      if ((s > i && (o = 1 / o), o)) {
        let r = !1
        let l = !1
        const c =
          (72 * parseFloat(getComputedStyle(e).getPropertyValue('font-size'))) /
          96
        if (
          (o < 4.5 &&
            ((r = !0),
            (c >= 18 ||
              (c >= 14 && e.style.fontWeight === 'bold') ||
              (c >= 14 &&
                getComputedStyle(e).getPropertyValue('font-weight') >= 700)) &&
              ((r = o < 3), (l = !0))),
          r)
        ) {
          const d = wave.engine.fn.waveRGBToHex(t)
          const u = wave.engine.fn.waveRGBToHex(a)
          const g = [wave.engine.fn.Dec2(Number(o)), d, u, l]
          wave.engine.contrast[e.getAttribute('data-waveid')] = {
            contrastData: g,
            element: e
          }
        }
  ...
  (wave.engine.fn.getLuminance = function (e) {
    e = e.slice()
    for (let t = 0; t < 3; t++) {
      let a = e[t]
      ;(a =
        (a /= 255) < 0.03928 ? a / 12.92 : Math.pow((a + 0.055) / 1.055, 2.4)),
        (e[t] = a)
    }
  ...
  (wave.engine.fn.Dec2 = function (e) {
    if ((e = String(e)).indexOf('.') !== -1) {
      const t = e.split('.')
      return t.length == 1
        ? Number(e)
  ...
  (wave.engine.fn.waveRGBToHex = function (e) {
    let t = (+e[0]).toString(16)
    let a = (+e[1]).toString(16)
    let n = (+e[2]).toString(16)
    let i = (+e[3]).toString(16)
    return (
      (i = ('0' + Math.round(255 * e[3]).toString(16)).slice(-2)),
      t.length == 1 && (t = '0' + t),
      a.length == 1 && (a = '0' + a),
      n.length == 1 && (n = '0' + n),
      i == 'ff' && (i = ''),
  ...
  (wave.engine.icons.docs = {
    alt_missing: {
      ...
      resources:
        'https://webaim.org/techniques/alttext/ Appropriate Use of Alternative Text [WebAIM]',
      icon_order: '1',
      position: 'after',
      page_rule: '0',
      guidelines: {
    ...
    list_possible: {
      ...
      details:
        'Text is used to present list-type content, such as:<br />\n* text<br />\n1. text<br />\na. text<br />\n1) text<br />\na) text<br />\n- text',
      resources: '',
      icon_order: '19',
      position: 'first',
      page_rule: '0',
    ...
    flash: {
      ...
      resources:
        '<a href="https://webaim.org/techniques/flash/techniques#hiding">hide it from screen readers</a>',
      icon_order: '31',
      position: 'after',
      page_rule: '0',
      guidelines: {
    ...
    figure: {
      ...
      purpose:
        'A &lt;figure&gt; represents self-contained content, potentially with an optional &lt;figcaption&gt;. Non-text content, if present, \nmust have alternative text.',
      actions:
...
;(wave.rules = {}),
  (wave.rules.alt_missing = function (e) {
    wave.engine.fn.overrideby('alt_missing', [
      'alt_link_missing',
      'alt_map_missing',
      'alt_spacer_missing'
    ]),
      e.querySelectorAll('img:not([alt])').forEach(function (e, t) {
        ;(e.getAttribute('title') != null &&
          e.getAttribute('title').length !== 0) ||
          (e.getAttribute('aria-label') != null &&
            e.getAttribute('aria-label').length !== 0) ||
          (e.getAttribute('aria-labelledby') != null &&
            e.getAttribute('aria-labelledby').length !== 0) ||
          e.getAttribute('role') === 'presentation' ||
          wave.engine.fn.addIcon(e, 'alt_missing')
      })
  }),
  (wave.rules.alt_link_missing = function (e) {
    wave.engine.fn.override('alt_link_missing', [
      'alt',
      'alt_missing',
      'alt_spacer_missing',
      'alt_null',
      ...
      e.querySelectorAll('a').forEach(function (e, t) {
        e.parentNode.id != 'wave5codetoggle' &&
          getComputedStyle(e).getPropertyValue('display') != 'none' &&
          (wave.engine.fn.hasTextContent(e) ||
            e.querySelectorAll('img').forEach(function (e, t) {
              wave.engine.fn.addIcon(e, 'alt_link_missing')
            }))
      })
  }),
  (wave.rules.alt_spacer_missing = function (e) {
    wave.engine.fn.override('alt_spacer_missing', ['alt_missing']),
      ...
      e.querySelectorAll('img:not([alt])').forEach(function (e, t) {
        ...
        ;((i &&
          (i = (i = (i = e.getAttribute('src').split('/'))[i.length - 1].split(
            '\\'
          ))[i.length - 1]).search(/^(spacer|space|blank)\..+$/i) !== -1) ||
          (a !== null && a > 0 && a <= 3) ||
          (n !== null && n > 0 && n <= 3)) &&
          wave.engine.fn.addIcon(e, 'alt_spacer_missing')
      })
  }),
  (wave.rules.alt_input_missing = function (e) {
    e.querySelectorAll('input[type=image]').forEach(function (e, t) {
      const a = e.getAttribute('alt')
      ;(a && a.search(/^ *$/) === -1) ||
        wave.engine.fn.addIcon(e, 'alt_input_missing')
    })
  }),
  (wave.rules.alt_area_missing = function (e) {
    e.querySelectorAll('area').forEach(function (t, a) {
      const n = t.getAttribute('alt')
      if (!n || n.search(/^ *$/) !== -1) {
        const i = t.closest('map').getAttribute('name')
        e.querySelectorAll('img[usemap=\\#' + i + ']').forEach(function (e, t) {
          wave.engine.fn.addIcon(e, 'alt_area_missing')
        })
      }
    })
  }),
  (wave.rules.alt_map_missing = function (e) {
    wave.engine.fn.override('alt_map_missing', ['alt_missing']),
      e.querySelectorAll('img[usemap]:not([alt])').forEach(function (e, t) {
        wave.engine.fn.addIcon(e, 'alt_map_missing')
      })
  }),
  (wave.rules.longdesc_invalid = function (e) {
    wave.engine.fn.override('longdesc_invalid', ['longdesc']),
      e.querySelectorAll('[longdesc]').forEach(function (e, t) {
        const a = new RegExp('(([a-zA-Z0-9]|\\|/))+')
        const n = new RegExp('(.html|.htm)$')
        const i = new RegExp('(.jpg|.gif|.png)$')
        const s = e.getAttribute('longdesc')
        a.test(s) &&
          (s.length == 0 ||
            i.test(s) ||
            (s.indexOf(' ') != -1 && !n.test(s))) &&
          wave.engine.fn.addIcon(e, 'longdesc_invalid', s)
      })
  }),
  (wave.rules.select_missing_label = function (e) {
    wave.engine.fn.override('select', ['label_missing']),
      e.querySelectorAll('select').forEach(function (t, a) {
        t.options.length > 0 &&
          !wave.engine.fn.isLabeled(t, e) &&
          t.options[t.selectedIndex] &&
          t.options[t.selectedIndex].text.length > 0 &&
          wave.engine.fn.addIcon(t, 'select_missing_label')
      })
  }),
  (wave.rules.label_missing = function (e) {
    wave.engine.fn.overrideby('label_missing', ['select_missing_label']),
      e
        .querySelectorAll(
          'input:not([type=image],[type=submit],[type=Submit],[type=SUBMIT],[type=reset],[type=Reset],[type=button],[type=hidden],[type=HIDDEN]), select, textarea'
        )
        .forEach(function (t, a) {
          wave.engine.fn.isLabeled(t, e) ||
            wave.engine.fn.addIcon(t, 'label_missing')
        })
  }),
  (wave.rules.label_empty = function (e) {
    wave.engine.fn.override('label_empty', ['label', 'label_orphaned']),
      e.querySelectorAll('label').forEach(function (t, a) {
        const n = t.querySelectorAll(
          'input, select, textarea, meter, output, progress'
        )
        const i = t.getAttribute('for')
        ;(!wave.engine.fn.hasTextContent(t) ||
          (((n.length > 0 && wave.engine.fn.isLabeled(n[0], e)) || i != null) &&
            !wave.engine.fn.hasTextContent(t))) &&
          wave.engine.fn.addIcon(t, 'label_empty')
      })
  }),
  (wave.rules.label_multiple = function (e) {
    e.querySelectorAll(
      'input:not([type=image],[type=submit],[type=reset],[type=button],[type=hidden]), select, textarea'
    ).forEach(function (t, a) {
      const n = t.getAttribute('id')
      const i = t.closest('label')
      const s = i != null ? i.length : 0
      const o = n != null ? e.querySelectorAll("label[for='" + n + "']") : null
      o &&
        (o.length > 1 || (s == 1 && o.length == 1 && i[0] !== o[0])) &&
        wave.engine.fn.addIcon(t, 'label_multiple')
    })
  }),
  (wave.rules.title_invalid = function (e) {
    const t = e.querySelector('title')
    if (t == null) return wave.engine.fn.addPagewideIcon('title_invalid')
    t.parentNode.nodeName != 'svg' &&
      ((t.textContent != null &&
        t.length !== 0 &&
        t.textContent.length !== 0 &&
        t.textContent.search(/(^ +$)|(^untitled.*$)/i) === -1) ||
        wave.engine.fn.addPagewideIcon('title_invalid'))
  }),
  (wave.rules.language_missing = function (e) {
    wave.engine.fn.override('language_missing', ['lang'])
    const t = [
      'aa',
      'ab',
      'ae',
    ...
    e.querySelectorAll('html:not([lang])').forEach(function (e, t) {
      wave.engine.fn.addPagewideIcon('language_missing')
    }),
      e.querySelectorAll('[lang]').forEach(function (e, n) {
        let i = !1
        const s = e.getAttribute('lang').toLowerCase()
        if (s.indexOf('-') > -1) {
          const o = s.split('-')[0]
          t.includes(o) || (i = !0)
          const r = s.split('-')[1]
          a.includes(r) || (i = !0)
        } else t.includes(s) || (i = !0)
        i &&
          (e.nodeName == 'HTML'
            ? wave.engine.fn.addPagewideIcon('language_missing', s)
            : wave.engine.fn.addIcon(e, 'language_missing', s))
      })
  }),
  (wave.rules.meta_refresh = function (e) {
    e.querySelectorAll('meta[http-equiv="refresh"]').forEach(function (e, t) {
      wave.engine.fn.addPagewideIcon('meta_refresh')
    })
  }),
  (wave.rules.heading_empty = function (e) {
    e.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(function (e, t) {
      wave.engine.fn.hasTextContent(e) ||
        wave.engine.fn.addIcon(e, 'heading_empty')
    })
  }),
  (wave.rules.button_empty = function (e) {
    wave.engine.fn.overrideby('button_empty', [
      'alt_link_missing',
      'aria_label'
    ]),
      e
        .querySelectorAll("button:not([type='reset'], [role='button']")
        .forEach(function (e, t) {
          wave.engine.fn.hasTextContent(e) ||
            e.title.length != 0 ||
            e.getAttribute('tabindex') === '-1' ||
            wave.engine.fn.addIcon(e, 'button_empty')
        }),
      e
        .querySelectorAll(
  ...
  (wave.rules.link_empty = function (e) {
    wave.engine.fn.overrideby('link_empty', ['alt_link_missing', 'aria_label']),
      e.querySelectorAll('a[href]').forEach(function (e, t) {
        e.parentNode.id != 'wave5codetoggle' &&
          (e.querySelectorAll('img').length != 0 ||
            wave.engine.fn.hasTextContent(e) ||
            e.title.length != 0 ||
            e.getAttribute('tabindex') === '-1' ||
            wave.engine.fn.addIcon(e, 'link_empty'))
      })
  }),
  (wave.rules.link_skip_broken = function (e) {
    wave.engine.fn.override('link_skip_broken', [
      'link_skip',
      'link_internal_broken'
    ])
    e.querySelectorAll("a[href^='#']").forEach(function (t, a) {
      let n = !1
      const i = t.getAttribute('href')
      if (i === '#') return !1
      if (t.textContent.trim().search(/^(skip|jump).*/gi) !== -1) {
        if (
          getComputedStyle(t).getPropertyValue('display') == 'none' ||
          getComputedStyle(t).getPropertyValue('visibility') == 'hidden'
        )
          n = !0
        else {
          const s = i.replace('#', '')
          e.querySelectorAll('[id="' + s + '"]').length === 0 &&
            e.querySelectorAll('a[name="' + s + '"]').length === 0 &&
            (n = !0)
        }
        n && wave.engine.fn.addIcon(t, 'link_skip_broken')
      }
    })
  }),
  (wave.rules.th_empty = function (e) {
    e.querySelectorAll('th').forEach(function (e, t) {
      wave.engine.fn.hasTextContent(e) || wave.engine.fn.addIcon(e, 'th_empty')
    })
  }),
  (wave.rules.blink = function (e) {
    e
      .querySelectorAll(
        'p, span, div, td, th, dd, ul, ol, h1, h2, h3, h4, h5, h6, blink'
      )
      .forEach(function (e, t) {
        const a = e.getAttribute('style')
        const n = new RegExp('s*text-decorations*:s*blinks*')
        a && n.test(a) && wave.engine.fn.addIcon(e, 'blink')
      }),
      e.querySelectorAll('blink').forEach(function (e, t) {
        wave.engine.fn.addIcon(e, 'blink')
      })
  }),
  (wave.rules.marquee = function (e) {
    e.querySelectorAll('marquee').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'marquee')
    })
  }),
  (wave.rules.contrast = function (e) {
    for (const t in (wave.engine.fn.calculateContrast(), wave.engine.contrast))
      wave.engine.fn.addIcon(
        wave.engine.contrast[t].element,
        'contrast',
        wave.engine.contrast[t].contrastData
      )
  }),
  (wave.rules.alt_redundant = function (e) {
    wave.engine.fn.override('alt_redundant', ['alt']),
      ...
      e.querySelectorAll("img:not([alt=''])[alt]").forEach(function (e, t) {
        const a = e.getAttribute('alt')
        if (
          !(
            (a && a.length > 0 && a.search(/^ +$/i) !== -1) ||
            (a && a.length < 3)
          )
        ) {
          for (var n = '', i = e; n.length < 15; )
            if (
              (i.nodeType == i.TEXT_NODE && (n += i.nodeValue.toLowerCase()),
              i.lastChild)
            )
              i = i.lastChild
          ...
          for (i = e; s.length < 15; )
            if (
              (i.nodeType == i.TEXT_NODE && (s += i.nodeValue.toLowerCase()),
              i.firstChild)
            )
              i = i.firstChild
          ...
          ;(n = n.substring(
            n.length - 15 - a.toLowerCase().length,
            n.length - 1
          )),
            (s = s.substring(0, 14 + a.toLowerCase().length)),
            (n.indexOf(a.toLowerCase()) == -1 &&
              s.indexOf(a.toLowerCase()) == -1) ||
              wave.engine.fn.addIcon(e, 'alt_redundant', a)
        }
      })
  }),
  (wave.rules.alt_suspicious = function (e) {
    wave.engine.fn.override('alt_suspicious', ['alt', 'alt_redundant']),
      e.querySelectorAll('img[alt]').forEach(function (e, t) {
        const a = e.getAttribute('alt')
        a &&
          a.length > 0 &&
          (a.search(
            /^(image|graphic|photo|photograph|drawing|painting|artwork|logo|bullet|button|arrow|more|spacer|blank|chart|table|diagram|graph|\*)$/i
          ) !== -1 ||
            a.search(
              /(^(graphic of|bullet|image of).*)|(.*(image|graphic)$)|(^ +$)|.*(\.png|\.gif|\.jpeg|\.jpg|\.svg|\.bmp)$/i
            ) !== -1) &&
          wave.engine.fn.addIcon(e, 'alt_suspicious', a)
      })
  }),
  (wave.rules.alt_duplicate = function (e) {
    wave.engine.fn.override('alt_duplicate', ['alt', 'alt_missing'])
    for (
      let t = e.querySelectorAll("img, input[type='image']"), a = 0;
      a < t.length;
      a++
    )
      if (
        t[a].hasAttribute('alt') &&
        t[a].getAttribute('alt') != '' &&
        t[a].offsetWidth > 0 &&
        t[a].offsetHeight > 0
      )
        for (let n = a + 1; n <= a + 2 && n < t.length; n++)
          if (
            t[n].hasAttribute('alt') &&
            t[n].getAttribute('alt').trim().toLowerCase() ==
              t[a].getAttribute('alt').trim().toLowerCase() &&
            t[n].hasAttribute('src') &&
            t[n].getAttribute('src').toLowerCase() !==
              t[a].getAttribute('src').toLowerCase() &&
            t[n].offsetWidth > 0 &&
            t[n].offsetHeight > 0
          ) {
            wave.engine.fn.addIcon(
  ...
  (wave.rules.alt_long = function (e) {
    e.querySelectorAll('img[alt]').forEach(function (e, t) {
      const a = e.getAttribute('alt')
      a && a.length > 100 && wave.engine.fn.addIcon(e, 'alt_long', a)
    })
  }),
  (wave.rules.image_title = function (e) {
    e.querySelectorAll("img[title]:not([title=''])").forEach(function (e, t) {
      if (!e.getAttribute('alt') || e.getAttribute('alt').length == 0) {
        const a = e.getAttribute('title')
        wave.engine.fn.addIcon(e, 'image_title', a)
      }
    })
  }),
  (wave.rules.label_orphaned = function (e) {
    wave.engine.fn.overrideby('label_orphaned', ['label_empty']),
      e.querySelectorAll('label').forEach(function (t, a) {
        const n = t.querySelectorAll(
          'input, select, textarea, meter, output, progress'
        )
        const i = t.getAttribute('for')?.replace("'", '')
        ;((n.length == 0 && !i) ||
          (i &&
            e.querySelectorAll(
              "input[id='" +
          ...
          wave.engine.fn.addIcon(t, 'label_orphaned')
      })
  }),
  (wave.rules.fieldset_missing = function (e) {
    const t = {}
    for (const a in (e
      .querySelectorAll('input[name][type=checkbox], input[name][type=radio]')
      .forEach(function (e, a) {
        const n = e.closest('fieldset')
        if (!n || e.compareDocumentPosition(n) == 20) {
          const i = e.closest('[role=radiogroup]')
          if (!i || e.compareDocumentPosition(i) == 20) {
            const s = e.getAttribute('name')
            t[s] == null && (t[s] = []), t[s].length < 2 && t[s].push(e)
          }
        }
      }),
    t))
      if (t[a].length == 2) {
        const n = t[a][0].getAttribute('id')
        const i = t[a][0].closest('label')
        if (i && i.length > 0)
          wave.engine.fn.addIcon(i.get(0), 'fieldset_missing')
        else if (n) {
          const s = e.querySelectorAll("label[for='" + n + "']")
          s.length > 0 && wave.engine.fn.addIcon(s[0], 'fieldset_missing')
        } else wave.engine.fn.addIcon(t[a][0], 'fieldset_missing')
      }
  }),
  (wave.rules.legend_missing = function (e) {
    wave.engine.fn.override('legend_missing', ['fieldset']),
      e.querySelectorAll('fieldset').forEach(function (e, t) {
        e.querySelectorAll('legend').length === 0 &&
          wave.engine.fn.addIcon(e, 'legend_missing')
      })
  }),
  (wave.rules.label_title = function (e) {
    e.querySelectorAll(
      'input[title]:not([type=image],[type=submit],[type=Submit],[type=SUBMIT],[type=reset],[type=Reset],[type=button],[type=hidden],[type=HIDDEN]), select[title], textarea[title]'
    ).forEach(function (t, a) {
      !t.getAttribute('aria-label') &&
        !t.getAttribute('aria-labelledby') &&
        !wave.engine.fn.isLabeledByTag(t, e) &&
        t.getAttribute('title').length > 0 &&
        wave.engine.fn.addIcon(t, 'label_title')
    })
  }),
  (wave.rules.heading_missing = function (e) {
    wave.engine.fn.override('heading_missing', ['h1_missing']),
      e.querySelectorAll('h1, h2, h3, h4, h5, h6').length === 0 &&
        wave.engine.fn.addPagewideIcon('heading_missing')
  }),
  (wave.rules.region_missing = function (e) {
    e.querySelectorAll(
      'header, nav, main, footer, aside, [role=banner], [role=navigation], [role=main], [role=contentinfo], [role=region]:not(#wave5bottombar)'
    ).length === 0 && wave.engine.fn.addPagewideIcon('region_missing')
  }),
  (wave.rules.h1_missing = function (e) {
    wave.engine.fn.overrideby('h1_missing', ['heading_missing']),
      e.querySelectorAll('h1').length === 0 &&
        wave.engine.fn.addPagewideIcon('h1_missing')
  }),
  (wave.rules.heading_skipped = function (e) {
    let t = 0
    let a = 0
    e.querySelectorAll('h1, h2, h3, h4, h5, h6').forEach(function (e, n) {
      ;(a = Number(e.nodeName[1])),
        t != 0 && t - a < -1 && wave.engine.fn.addIcon(e, 'heading_skipped'),
        (t = a)
    })
  }),
  (wave.rules.heading_possible = function (e) {
    e.querySelectorAll('p').forEach(function (e, t) {
      const a = parseInt(getComputedStyle(e).getPropertyValue('font-size'), 10)
        ...
        (a > 20 ||
          (a > 16 &&
            (getComputedStyle(e).getPropertyValue('font-weight') >= 700 ||
              ...
              getComputedStyle(e).getPropertyValue('font-style') ==
                'italic'))) &&
        wave.engine.fn.addIcon(e, 'heading_possible')
    })
  }),
  (wave.rules.table_caption_possible = function (e) {
    e.querySelectorAll('table').forEach(function (e, t) {
      if (
        e.querySelectorAll('th').length > 0 &&
        e.querySelectorAll('caption').length == 0
      ) {
        const a = e.previousElementSibling
        const n = e.querySelectorAll('td')
        if (n.length > 0) {
          const i = n[0].getAttribute('colspan')
          i && i >= 3
            ? wave.engine.fn.addIcon(n[0], 'table_caption_possible')
            : a &&
              a.nodeName == 'P' &&
              (a.textContent.length < 50 ||
                (a.textContent.length < 100 &&
                  getComputedStyle(a).getPropertyValue('font-weight') >= 700 &&
                  getComputedStyle(a).getPropertyValue('text-align') ==
                    'center')) &&
              wave.engine.fn.addIcon(a, 'table_caption_possible')
        }
      }
    })
  }),
  (wave.rules.list_possible = function (e) {
    let t = ''
    e.querySelectorAll('p').forEach(function (e, a) {
      const n = e.textContent.trim().substr(0, 2)
      if (
        n != t &&
        ((t = n), n.search(/^(\*\s|\-\s|\-|1\.|a\.|1\)|a\))/) == 0)
      ) {
        e.nextElementSibling?.textContent
          ?.trim()
          .search(/^(\*\s|\-\s|\-|2\.|b\.|2\)|b\))/) == 0 &&
          wave.engine.fn.addIcon(e, 'list_possible')
        const i = e.innerHTML.split(/<br\s*.*?\/?>[\n\r]*\s*/gi, 2)
        i[1] &&
          i[1].trim().search(/^(\*\s|\-\s|\-|2\.|b\.|2\)|b\))/) == 0 &&
          wave.engine.fn.addIcon(e, 'list_possible')
      }
    })
  }),
  (wave.rules.link_internal_broken = function (e) {
    wave.engine.fn.overrideby('link_internal_broken', ['link_skip_broken']),
      e
        .querySelectorAll("a[href^='#']:not(a[href^='#/'],[href^='#top']")
        .forEach(function (t, a) {
          const n = t.getAttribute('href')
          const i = n.replace('#', '')
          n[0] &&
            n != '#' &&
            e.querySelectorAll('[id="' + i + '"]').length === 0 &&
            e.querySelectorAll('a[name="' + i + '"]').length === 0 &&
            wave.engine.fn.addIcon(t, 'link_internal_broken')
        })
  }),
  (wave.rules.link_suspicious = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      ...
      !a ||
        (a.search(/(click)/i) === -1 &&
          a.search(
            /^(here|more|more\.\.\.|details|more details|link|this page|continue|continue reading|read more|button)$/i
          ) === -1) ||
        wave.engine.fn.addIcon(e, 'link_suspicious')
    })
  }),
  (wave.rules.link_redundant = function (e) {
    let t = null
    e.querySelectorAll("a[href]:not(a[href^='#'])").forEach(function (e, a) {
      t && t.search(/^\s*$/i) && t == e.getAttribute('href')
        ? ((t = e.getAttribute('href')),
          wave.engine.fn.addIcon(e, 'link_redundant'))
        : (t = e.getAttribute('href'))
    })
  }),
  (wave.rules.link_word = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      ...
      a &&
        a.search(/(\.doc|\.docx)$/i) !== -1 &&
        wave.engine.fn.addIcon(e, 'link_word')
    })
  }),
  (wave.rules.link_excel = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      const a = e.getAttribute('href')
      a &&
        a.search(/(\.xls|\.xlsx)$/i) !== -1 &&
        wave.engine.fn.addIcon(e, 'link_excel')
    })
  }),
  (wave.rules.link_powerpoint = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      const a = e.getAttribute('href')
      a &&
        a.search(/(\.ppt|\.pptx|\.pps|\.ppsx)$/i) !== -1 &&
        wave.engine.fn.addIcon(e, 'link_powerpoint')
    })
  }),
  (wave.rules.link_pdf = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      const a = e.getAttribute('href')
      a && a.search(/(\.pdf)$/i) !== -1 && wave.engine.fn.addIcon(e, 'link_pdf')
    })
  }),
  (wave.rules.link_document = function (e) {
    e.querySelectorAll('a[href]').forEach(function (e, t) {
      ...
      a &&
        a.search(
          /(\.rtf|\.wpd|\.ods|\.odt|\.odp|\.sxw|\.sxc|\.sxd|\.sxi|\.pages|\.key)$/i
        ) !== -1 &&
        wave.engine.fn.addIcon(e, 'link_document')
    })
  }),
  (wave.rules.audio_video = function (e) {
    wave.engine.fn.override('audio_video', ['plugin']),
      e.querySelectorAll('embed').forEach(function (e, t) {
        const a = e.getAttribute('src')
        a &&
          a.search(
            /(\.mov|\.asx|\.wvx|\.wax|\.wmv|\.wma|\.ram|\.rpm|\.ra|\.rm)$/i
          ) !== -1 &&
          wave.engine.fn.addIcon(e, 'audio_video')
      }),
      e
        .querySelectorAll(
      ...
      e.querySelectorAll('a[href]').forEach(function (e, t) {
        ...
        a &&
          a.search(
            /(\.3gp|\.aif|\.aiff|\.asf|\.asx|\.avi|\.flv|\.m4a|\.m4p|\.mov|\.mp2|\.mp3|\.mp4|\.mpa|\.mpeg|\.mpeg2|\.mpg|\.mpv|\.ogg|\.ogv|\.qtl|\.ra|\.ram|\.smi|\.smil|\.wav|\.wax|\.webm|\.wma|\.wmp|\.wmx)$/i
          ) !== -1 &&
          wave.engine.fn.addIcon(e, 'audio_video')
      })
  }),
  (wave.rules.flash = function (e) {
    wave.engine.fn.override('flash', ['plugin']),
      e
        .querySelectorAll(
          "embed[src], embed[type='application/x-shockwave-flash']"
        )
        .forEach(function (e, t) {
          const a = e.getAttribute('src')
          if (a && a.search(/(\.swf)$/i) !== -1)
            wave.engine.fn.addIcon(e, 'flash')
          else {
      ...
      e
        .querySelectorAll(
          "object[type='application/x-shockwave-flash'], object[classid='clsid:D27CDB6E-AE6D-11cf-96B8-444553540000'], object[classid='clsid:d27cdb6e-ae6d-11cf-96b8-444553540000']"
        )
        .forEach(function (e, t) {
          wave.engine.fn.addIcon(e, 'flash')
        })
  ...
  (wave.rules.applet = function (e) {
    e.querySelectorAll('applet').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'applet')
    })
  }),
  (wave.rules.plugin = function (e) {
    wave.engine.fn.overrideby('plugin', ['flash', 'audio_video']),
      e.querySelectorAll('object,embed').forEach(function (e, t) {
        wave.engine.fn.addIcon(e, 'plugin')
      })
  }),
  (wave.rules.noscript = function (e) {
    e.querySelectorAll('noscript').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'noscript')
    })
  }),
  (wave.rules.event_handler = function (e) {
    e.querySelectorAll('[onmouseover],[onclick],[ondblclick]').forEach(
      function (e, t) {
        const a = e.getAttribute('onmouseover')
        const n = e.getAttribute('onfocus')
        const i = e.getAttribute('onclick')
        const s = e.getAttribute('ondblclick')
        const o = e.nodeName.toLowerCase()
        const r = e.getAttribute('tabindex')
        ;(a && !n) || s
          ? wave.engine.fn.addIcon(e, 'event_handler')
          : i &&
            o !== 'a' &&
            o !== 'input' &&
            o !== 'button' &&
            ...
            o !== 'option' &&
            o !== 'textarea' &&
            ((r && r === '0') || wave.engine.fn.addIcon(e, 'event_handler'))
      }
    )
  }),
  (wave.rules.javascript_jumpmenu = function (e) {
    e.querySelectorAll('select[onchange]').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'javascript_jumpmenu')
    })
  }),
  (wave.rules.accesskey = function (e) {
    e.querySelectorAll('[accesskey]').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'accesskey', e.getAttribute('accesskey'))
    })
  }),
  (wave.rules.tabindex = function (e) {
    e.querySelectorAll('[tabindex]').forEach(function (e, t) {
      const a = e.getAttribute('tabindex')
      Number(a) !== 'NaN' &&
        Number(a) > 0 &&
        wave.engine.fn.addIcon(e, 'tabindex')
    })
  }),
  (wave.rules.text_small = function (e) {
    e.querySelectorAll(
      'p, span:not(.wavehidden), div:not(#wave5spinner), td, th, dd, ul, ol, h1, h2, h3, h4, h5, h6, font, i'
    ).forEach(function (e, t) {
      const a = parseInt(getComputedStyle(e).getPropertyValue('font-size'), 10)
      wave.engine.fn.elementHasText(e) &&
        a <= 10 &&
        wave.engine.fn.addIcon(e, 'text_small')
    })
  }),
  (wave.rules.text_justified = function (e) {
    e.querySelectorAll('p, div, td').forEach(function (e, t) {
      getComputedStyle(e).getPropertyValue('text-align') == 'justify' &&
        wave.engine.fn.addIcon(e, 'text_justified')
    })
  }),
  (wave.rules.underline = function (e) {
    e.querySelectorAll('p, span, div, td, th, dd, li, u').forEach(
      function (e, t) {
        e.nodeName == 'U' && e.parentNode.nodeName != 'A'
          ? wave.engine.fn.addIcon(e, 'underline')
          : (getComputedStyle(e).getPropertyValue('text-decoration') !=
              'underline' &&
              getComputedStyle(e).getPropertyValue('text-decoration-line') !=
                'underline') ||
            wave.engine.fn.addIcon(e, 'underline')
      }
    )
  ...
  (wave.rules.title_redundant = function (e) {
    e.querySelectorAll('[title]:not(img)').forEach(function (e, t) {
      const a = e.getAttribute('title')
      a &&
        a.length > 0 &&
        e.textContent?.trim() == a.trim() &&
        wave.engine.fn.addIcon(e, 'title_redundant')
    }),
      e.querySelectorAll('img[title]').forEach(function (e, t) {
        const a = e.getAttribute('title')
        a &&
          a.length > 0 &&
          e.getAttribute('alt')?.trim() == a.trim() &&
          wave.engine.fn.addIcon(e, 'title_redundant')
      })
  }),
  (wave.rules.alt = function (e) {
    wave.engine.fn.overrideby('alt', [
      'alt_redundant',
      'alt_duplicate',
      'alt_link',
      'alt_link_missing',
    ...
    wave.engine.fn.override('alt_spacer', ['alt', 'alt_missing', 'alt_null']),
      ...
      e.querySelectorAll("img[alt='']").forEach(function (e, t) {
        if (e.getAttribute('alt') == '') {
          ...
          let i = e.getAttribute('src')
          ;((i &&
            (i = (i = (i = e.getAttribute('src').split('/'))[
              i.length - 1
            ].split('\\'))[i.length - 1]).search(
              /^(spacer|space|blank)\..+$/i
            ) !== -1) ||
            (a !== null && a > 0 && a <= 3) ||
            (n !== null && n > 0 && n <= 3)) &&
            wave.engine.fn.addIcon(e, 'alt_spacer')
        }
      })
  }),
      ...
      e.querySelectorAll('a').forEach(function (e, t) {
        if (
          ...
          e.querySelectorAll('img[alt]').forEach(function (e, t) {
            ...
            if (n && n.length > 0 && n.length <= 100) {
              if (a !== null) return void (a = null)
              a = e
            }
          }),
            a !== null &&
              wave.engine.fn.addIcon(a, 'alt_link', a.getAttribute('alt'))
        }
      })
  }),
  (wave.rules.alt_input = function (e) {
    e.querySelectorAll("input[type='image'][alt]").forEach(function (e, t) {
      const a = e.getAttribute('alt')
      a && a.length > 0 && wave.engine.fn.addIcon(e, 'alt_input', a)
    })
  }),
  (wave.rules.alt_map = function (e) {
    wave.engine.fn.override('alt_map', ['alt', 'alt_missing']),
      e.querySelectorAll('img[alt][usemap]').forEach(function (e, t) {
        const a = e.getAttribute('alt')
        a && a.length > 0 && a.search(/^ +$/i) === -1
          ? wave.engine.fn.addIcon(e, 'alt_map', a)
          : wave.engine.fn.addIcon(e, 'alt_map')
      })
  }),
  (wave.rules.alt_area = function (e) {
    e.querySelectorAll('area').forEach(function (t, a) {
      ...
      if (n && n.search(/^\s*$/) == -1) {
        const i = t.closest('map').getAttribute('name')
        e.querySelectorAll("img[usemap='#" + i + "']").forEach(function (e, t) {
          wave.engine.fn.addIcon(e, 'alt_area')
        })
      }
    })
  }),
  (wave.rules.longdesc = function (e) {
    wave.engine.fn.overrideby('longdesc', ['longdesc_invalid']),
      e.querySelectorAll('[longdesc]').forEach(function (e, t) {
        const a = e.getAttribute('longdesc')
        wave.engine.fn.addIcon(e, 'longdesc', a)
      })
  }),
  (wave.rules.figure = function (e) {
    e.querySelectorAll('figure').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'figure')
    })
  }),
  (wave.rules.label = function (e) {
    wave.engine.fn.overrideby('label', ['label_empty']),
      e.querySelectorAll('label').forEach(function (t, a) {
        if (wave.engine.fn.hasTextContent(t)) {
          const n = t.querySelectorAll(
            'input, select, textarea, meter, output, progress'
          )
          const i = t.getAttribute('for')?.replace("'", '')
          ;((n.length > 0 && wave.engine.fn.isLabeled(n[0], e)) ||
            (i &&
              e.querySelectorAll(
                "input[id='" +
                  i +
            ...
            wave.engine.fn.addIcon(t, 'label')
        }
      })
  }),
  (wave.rules.fieldset = function (e) {
    wave.engine.fn.overrideby('fieldset', ['legend_missing']),
      e.querySelectorAll('fieldset').forEach(function (e, t) {
        const a = e.querySelector('legend')
        wave.engine.fn.addIcon(a != null ? a : e, 'fieldset')
      })
  }),
  (wave.rules.link_skip = function (e) {
    wave.engine.fn.overrideby('link_skip', ['link_skip_broken']),
      e.querySelectorAll('a[href]').forEach(function (t, a) {
        const n = t.getAttribute('href')
        if (n && n.length != 0 && n[0] == '#' && n != '#') {
          const i = n.replace('#', '')
          t.textContent.trim().search(/^(skip|jump).*/gi) !== -1 &&
            n &&
            (e.querySelectorAll(n).length > 0 ||
              e.querySelectorAll('[name="' + i + '"]').length > 0) &&
            wave.engine.fn.addIcon(t, 'link_skip')
        }
      })
  }),
      ...
      if (n && n.length != 0 && n[0] == '#' && n != '#') {
        ...
        t.textContent.trim().search(/^(skip|jump).*/gi) !== -1 &&
          (n &&
            e.querySelectorAll(n).length > 0 &&
            wave.engine.fn.addIcon(
              e.querySelectorAll(n)[0],
              'link_skip_target'
            ),
          e.querySelectorAll('[name="' + i + '"]').length > 0 &&
            wave.engine.fn.addIcon(
              e.querySelector('[name="' + i + '"]'),
              'link_skip_target'
            ))
      }
    })
  ...
  (wave.rules.lang = function (e) {
    wave.engine.fn.overrideby('lang', ['language_missing']),
      e.querySelectorAll('[lang]').forEach(function (e, t) {
        const a = e.getAttribute('lang')
        a &&
          a.length > 0 &&
          (e.nodeName == 'HTML'
            ? wave.engine.fn.addPagewideIcon('lang', a)
            : wave.engine.fn.addIcon(e, 'lang', a))
      })
  }),
  (wave.rules.table_layout = function (e) {
    e.querySelectorAll('table').forEach(function (e, t) {
      let a = !1
      if (
        (e.querySelectorAll('th').forEach(function (t, n) {
          const i = t.closest('table')
          e.compareDocumentPosition(i) != 20 && (a = !0)
        }),
        a == 0)
      ) {
        const n = e.querySelectorAll('td')
        n.length > 0
          ? wave.engine.fn.addIcon(n[0], 'table_layout')
          : wave.engine.fn.addIcon(e, 'table_layout')
      }
    })
  }),
  (wave.rules.table_data = function (e) {
    e.querySelectorAll('table').forEach(function (e, t) {
      ...
      e.querySelectorAll('th').forEach(function (t, n) {
        ...
        a == 1 && wave.engine.fn.addIcon(e, 'table_data')
    })
  }),
  (wave.rules.table_caption = function (e) {
    e.querySelectorAll('caption').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'table_caption')
    })
  }),
  (wave.rules.th = function (e) {
    e.querySelectorAll('th:not([scope])').forEach(function (e, t) {
      wave.engine.fn.hasTextContent(e) && wave.engine.fn.addIcon(e, 'th')
    })
  }),
  (wave.rules.th_col = function (e) {
    e.querySelectorAll("th[scope='col']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'th_col')
    })
  }),
  (wave.rules.th_row = function (e) {
    e.querySelectorAll("th[scope='row']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'th_row')
    })
  }),
  (wave.rules.h1 = function (e) {
    e.querySelectorAll('h1').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h1')
    })
  }),
  (wave.rules.h2 = function (e) {
    e.querySelectorAll('h2').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h2')
    })
  }),
  (wave.rules.h3 = function (e) {
    e.querySelectorAll('h3').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h3')
    })
  }),
  (wave.rules.h4 = function (e) {
    e.querySelectorAll('h4').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h4')
    })
  }),
  (wave.rules.h5 = function (e) {
    e.querySelectorAll('h5').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h5')
    })
  }),
  (wave.rules.h6 = function (e) {
    e.querySelectorAll('h6').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'h6')
    })
  }),
  (wave.rules.ol = function (e) {
    e.querySelectorAll('ol').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'ol')
    })
  }),
  (wave.rules.ul = function (e) {
    e.querySelectorAll('ul').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'ul')
    })
  }),
  (wave.rules.dl = function (e) {
    e.querySelectorAll('dl').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'dl')
    })
  }),
  (wave.rules.iframe = function (e) {
    e.querySelectorAll('iframe').forEach(function (e, t) {
      e.id != 'wave_sidebar_container' && wave.engine.fn.addIcon(e, 'iframe')
    })
  }),
  (wave.rules.header = function (e) {
    e.querySelectorAll("header,[role='banner']").forEach(function (e, t) {
      const a = e.closest('article')
      if (!a || e.compareDocumentPosition(a) == 20) {
        const n = e.closest('section')
        ;(n && e.compareDocumentPosition(n) != 20) ||
          wave.engine.fn.addIcon(e, 'header')
      }
    })
  }),
  (wave.rules.nav = function (e) {
    e.querySelectorAll("nav,[role='navigation']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'nav')
    })
  }),
  (wave.rules.search = function (e) {
    e.querySelectorAll("[role='search']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'search')
    })
  }),
  (wave.rules.main = function (e) {
    e.querySelectorAll("main,[role='main']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'main')
    })
  }),
  (wave.rules.footer = function (e) {
    e.querySelectorAll("footer,[role='contentinfo']").forEach(function (e, t) {
      const a = e.closest('article')
      if (!a || e.compareDocumentPosition(a) == 20) {
        const n = e.closest('section')
        ;(n && e.compareDocumentPosition(n) != 20) ||
          wave.engine.fn.addIcon(e, 'footer')
      }
    })
  }),
  (wave.rules.aside = function (e) {
    e.querySelectorAll("aside,[role='complementary']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'aside')
    })
  }),
  (wave.rules.region = function (e) {
    wave.engine.fn.override('region', ['region_missing']),
      e
        .querySelectorAll("[role='region']:not(#wave5bottombar)")
        .forEach(function (e, t) {
          const a = e.getAttribute('aria-label')
          const n = e.getAttribute('aria-labelledby')
          ;(a == null && n == null) || wave.engine.fn.addIcon(e, 'region')
        })
  }),
  (wave.rules.html5_video_audio = function (e) {
    e.querySelectorAll('audio, video').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'html5_video_audio')
    })
  }),
  (wave.rules.youtube_video = function (e) {
    e
      .querySelectorAll(
        "iframe[src*='youtube.com'],iframe[data-src*='youtube.com'],iframe[src*='youtube-nocookie.com'],iframe[data-src*='youtube-nocookie.com']"
      )
      .forEach(function (e, t) {
        wave.engine.fn.addIcon(e, 'youtube_video')
      }),
      e.querySelectorAll("a[href*='youtube.com']").forEach(function (e, t) {
        const a = e
          .getAttribute('href')
          .split(/(vi\/|v%3D|v=|\/v\/|youtu\.be\/|\/embed\/)/)
        const n = void 0 !== a[2] ? a[2].split(/[^0-9a-z_\-]/i)[0] : a[0]
        n.includes('youtube') ||
          n.includes('user') ||
          wave.engine.fn.addIcon(e, 'youtube_video')
      })
  ...
  (wave.rules.aria = function (e) {
    ...
    e.querySelectorAll('*:not(#wave5bottombar)').forEach(function (e, n) {
      for (var i = '', s = 0; s < e.attributes.length; s++)
        (e.attributes[s].name.search(/^aria-/i) !== -1 ||
          e.getAttribute('role')) &&
          ((e.attributes[s].name.search(/^aria-/i) !== -1 &&
            t.indexOf(e.attributes[s].name) < 0) ||
            (e.attributes[s].name == 'role' &&
              a.indexOf(e.attributes[s].value) < 0)) &&
          (i += e.attributes[s].name + '="' + e.attributes[s].value + '", ')
      i !== '' && wave.engine.fn.addIcon(e, 'aria', i.slice(0, -2))
    })
  }),
  (wave.rules.aria_label = function (e) {
    wave.engine.fn.override('aria_label', ['link_empty', 'button_empty']),
      e
        .querySelectorAll(
          '[aria-labelledby]:not(#wave5bottombar), [aria-label]:not(#wave5bottombar)'
        )
        .forEach(function (e, t) {
          for (var a = 0, n = 0; n < e.attributes.length; n++)
            if (e.attributes[n].name.search(/^aria-label/i) !== -1) {
              const i =
                e.attributes[n].name + '="' + e.attributes[n].value + '"'
              wave.engine.fn.addIcon(e, 'aria_label', i), a++
            }
          a == 0 && wave.engine.fn.addIcon(e, 'aria_label')
        })
  }),
  (wave.rules.aria_describedby = function (e) {
    e.querySelectorAll('[aria-describedby]').forEach(function (e, t) {
      for (var a = 0, n = 0; n < e.attributes.length; n++)
        if (e.attributes[n].name.search(/^aria-describedby/i) !== -1) {
          const i = e.attributes[n].name + '="' + e.attributes[n].value + '"'
          wave.engine.fn.addIcon(e, 'aria_describedby', i), a++
        }
      a == 0 && wave.engine.fn.addIcon(e, 'aria_describedby')
    })
  }),
  (wave.rules.aria_reference_broken = function (e) {
    wave.engine.fn.override('aria_reference_broken', [
      'aria_label',
      'aria_describedby'
    ])
    let t = !1
    e.querySelectorAll('*[aria-labelledby], *[aria-describedby]').forEach(
      function (a, n) {
        t = !1
        let i = a.getAttribute('aria-labelledby')
        if (i != null && i != '') {
          i = i.trim().split(' ')
          for (var s = 0; s < i.length; s++)
            if (e.querySelectorAll('[id="' + i[s] + '"]').length === 0) {
              t = !0
              break
            }
        }
        if (!t) {
          let o = a.getAttribute('aria-describedby')
          if (o != null && o != '') {
            o = o.trim().split(' ')
            for (s = 0; s < o.length; s++)
              if (e.querySelectorAll('[id="' + o[s] + '"]').length === 0) {
                t = !0
                break
              }
          }
        }
        t && wave.engine.fn.addIcon(a, 'aria_reference_broken')
      }
    )
  }),
  (wave.rules.aria_tabindex = function (e) {
    e.querySelectorAll('[tabindex]').forEach(function (e, t) {
      ...
      Number(a) !== 'NaN' &&
        Number(a) < 1 &&
        wave.engine.fn.addIcon(e, 'aria_tabindex', 'tabindex=' + a)
    })
  }),
  (wave.rules.aria_live_region = function (e) {
    e.querySelectorAll(
      "[role='alert'],[role='alertdialog'],[aria-live]"
    ).forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'aria_live_region')
    })
  }),
  (wave.rules.aria_menu_broken = function (e) {
    e.querySelectorAll(
      "[role='menu']:not([aria-busy='true']),[role='menubar']:not([aria-busy='true'])"
    ).forEach(function (e, t) {
      e.querySelectorAll(
        "[role='menuitem'], [role='menuitemcheckbox'], [role='menuitemradio']"
      ).length === 0 && wave.engine.fn.addIcon(e, 'aria_menu_broken')
    })
  }),
  (wave.rules.aria_menu = function (e) {
    wave.engine.fn.overrideby('aria_menu', ['aria_menu_broken']),
      e
        .querySelectorAll("[role='menu'],[role='menubar']")
        .forEach(function (e, t) {
          wave.engine.fn.addIcon(e, 'aria_menu')
        })
  }),
  (wave.rules.aria_button = function (e) {
    e.querySelectorAll("[role='button']").forEach(function (e, t) {
      e.nodeName !== 'BUTTON' && wave.engine.fn.addIcon(e, 'aria_button')
    })
  }),
  (wave.rules.aria_hidden = function (e) {
    e.querySelectorAll("[aria-hidden='true']").forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'aria_hidden')
    })
  }),
  (wave.rules.aria_expanded = function (e) {
    e.querySelectorAll(
      "[aria-expanded='true'],[aria-expanded='false']"
    ).forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'aria_expanded')
    })
  }),
  (wave.rules.aria_haspopup = function (e) {
    e.querySelectorAll('[aria-haspopup]').forEach(function (e, t) {
      wave.engine.fn.addIcon(e, 'aria_haspopup')
    })
  }),
  waveconfig.platform == 'extension'
    ? wave.init()
  ...
  (wave.fn.showCode = function () {
    wave.code.setHtmlRoot(wave.page.documentElement), wave.code.displayTree()
  }),
  (wave.fn.loadCodePanel = function (e) {
    wave.code.unhighlight()
    let t
    let a
    const n =
      waveconfig.platform == 'extension'
        ...
        : document.getElementById('report')
    Math.round(
      parseFloat(document.getElementById('wave5bottombar').offsetHeight)
    ) <= 1
      ? (document
          .getElementById('wave5codetoggle')
          .classList.toggle('wave5_open'),
        (t =
          waveconfig.platform == 'extension'
            ? n.offsetHeight + 200
            : n.offsetHeight - 200),
        (n.style.height = t + 'px'),
        (document.getElementById('wave5bottombar').style.height = '200px'),
        wave.tabsengine.codePanelHasRun ||
          (wave.fn.showCode(),
          (document.getElementById('wave5spinner').style.display = 'none'),
          (wave.tabsengine.codePanelHasRun = !0)),
        (wave.tabsengine.codePanelOpen = !0),
        waveconfig.platform == 'extension' &&
          (((a = document.getElementById('wave5bottombar')).style.float =
            'left'),
          (a.style.position = 'fixed'),
          (a.style.zIndex = '2147483647'),
          (a.style.borderTop = 'inset 1px #737c70'),
      ...
      : ((t =
          waveconfig.platform == 'extension'
            ...
            : n.offsetHeight + 200),
        (n.style.height = t + 'px'),
        (document.getElementById('wave5bottombar').style.height = '0px'),
        (wave.tabsengine.codePanelOpen = !1),
        document
          .getElementById('wave5codetoggle')
          .classList.toggle('wave5_open'),
  ...
  (wave.tabsengine.buildOutline = function (e) {
    if (
      (waveconfig.debug && console.log('Start buildOutline.'), void 0 === e)
    ) {
      waveconfig.debug &&
        console.log('outlineHtml undefined. Start buildOutlineHtml')
      e = wave.tabsengine.buildOutlineHtml()
      waveconfig.platform == 'extension'
        ? (waveconfig.debug &&
            console.log('Passing outline to handleOutlineData'),
          reportPageMessaging.fn.setupAndDispatchEvent('handleOutlineData', e))
        : wave.tabs.handleOutlineData(e)
    }
  }),
  (wave.tabsengine.buildOutlineHtml = function () {
    let e = ''
    const t = ['H1', 'H2', 'H3', 'H4', 'H5', 'H6']
    const a = ['HEADER', 'NAV', 'MAIN', 'ASIDE', 'FOOTER', 'SEARCH']
    const n = [
      'BANNER',
      'SEARCH',
      'NAVIGATION',
    ...
    for (
      var s = [],
        o = Object.keys(wave.report.iconlist.structure.items),
        r = [],
        l = 0;
      l < o.length;
      l++
      ...
      for (
        let c = 0;
        c < wave.report.iconlist.structure.items[o[l]].xpaths.length;
        c++
      )
        r.push(wave.report.iconlist.structure.items[o[l]].xpaths[c])
    for (const d in wave.report.things.allTagsCached)
      if (
        wave.report.things.allTagsCached.hasOwnProperty(d) &&
        r.indexOf(d) > -1
      ) {
        const u = wave.report.things.allTagsCached[d]
        let g = u.getAttribute('role')
        g = g !== null ? g.toUpperCase() : g
        var m
        const p = u.tagName.toUpperCase()
        const h = {
          HEADER: 'Header',
          NAV: 'Navigation',
          MAIN: 'Main',
        ...
        if (((m = h.hasOwnProperty(p) ? h[p] : i(g)), s.length > 0))
          for (l = 0; l < s.length; l++)
            d.indexOf(s[l]) == -1 && ((e += '</div>'), s.splice(l, 1), l--)
        if (wave.tabsengine.isHeaderTag(t, p))
          e +=
            "<div class='outline_" +
            p +
            "' data-reportxpath-heading='" +
            d +
            "'>" +
            wave.tabsengine.getAccName(u, !1, !1, !1, !0) +
            '</div>'
        ;(wave.tabsengine.isAriaLandmark(a, p) || n.indexOf(g) > -1) &&
          (s.push(d),
          (e +=
            "<div style='border: 1px solid #0050C7; padding: 10px; margin-bottom: 1px;'>"),
          (e +=
            "<div class='outline_aria' data-reportxpath-heading='" +
            d +
            "'>" +
            m +
            '</div>'))
  ...
  (wave.tabsengine.isHeaderTag = function (e, t) {
    return e.indexOf(t) > -1
  }),
  (wave.tabsengine.isAriaLandmark = function (e, t) {
    return e.indexOf(t) > -1
  }),
  (wave.tabsengine.toggleStyles = function (e) {
    const t = wave.page
    if ((wave.iconbox.hide(), !0 === e)) {
      t.querySelectorAll('table, tbody, td, tfoot, th, thead, tr').forEach(
        function (e) {
          ;(e.style.display = ''), (e.style.textAlign = '')
        }
      )
      var a = t.styleSheets
      for (let e = 0; e < a.length; e++) a[e].disabled = !1
      t.querySelectorAll('[wave5style]').forEach(function (e) {
        e.setAttribute('style', e.getAttribute('wave5style')),
          e.removeAttribute('wave5style')
      })
    } else if (!1 === e) {
      t.querySelectorAll(
        "svg:not(.wave5icon), img[src$='.svg']:not(.wave5icon)"
      ).forEach(function (e) {
        e.setAttribute('width', e.clientWidth),
          e.setAttribute('height', e.clientHeight)
      })
      a = t.styleSheets
      for (let e = 0; e < a.length; e++)
        (a[e].href == null ||
          (a[e].href != null &&
            a[e].href.indexOf('chrome-extension://') == -1 &&
            a[e].href.indexOf('moz-extension://') == -1)) &&
          (a[e].disabled = !0)
      t
        .querySelectorAll(
          '[style]:not(html,#wave_sidebar_container, #wave5topbar, #wave5bottombar, #wave5bottombar #wave5spinner, .wave5icon, .wave5text, .wave5text a, .waveorder)'
        )
        .forEach(function (e) {
          e.setAttribute('wave5style', e.getAttribute('style')),
            e.removeAttribute('style')
        }),
        t
          .querySelectorAll('table, tbody, td, tfoot, th, thead, tr')
          .forEach(function (e) {
            ;(e.style.display = 'block'), (e.style.textAlign = 'left')
          })
    }
    wave.tabsengine.navOrderVisible == 1 &&
      wave.tabsengine.showNavOrder('show', wave.tabsengine.WaveIconsVisible),
      wave.report.things.updateTopbar()
  }),
  (wave.tabsengine.toggleSaturation = function (e) {
    e == 'desaturate'
      ? document.documentElement.style.setProperty('filter', 'grayscale(100%)')
      : document.documentElement.style.setProperty('filter', '')
  }),
  (wave.tabsengine.showNavOrder = function (e, t) {
    if (
      (wave.iconbox.hide(),
      e != 'show' || t == null || t == wave.tabsengine.WaveIconsVisible)
    ) {
      const a =
        waveconfig.platform == 'extension'
          ? document
          : document.getElementById('report').contentWindow.document
      if (e == 'show') {
        a.getElementById('waveordercontainer') != null &&
          a.getElementById('waveordercontainer').remove(),
          (wave.tabsengine.navOrderVisible = !0)
        const n = wave.tabsengine.buildNavItems(a)
        waveconfig.platform == 'extension'
          ? (waveconfig.debug,
            reportPageMessaging.fn.setupAndDispatchEvent('handleNavData', n))
          : wave.tabs.handleNavData(n)
      } else
        (wave.tabsengine.navOrderVisible = !1),
          wave.tabsengine.showWaveIcons(!0),
          a.getElementById('waveordercontainer') != null &&
            a.getElementById('waveordercontainer').remove(),
          wave.report.things.updateTopbar()
    } else wave.tabsengine.showWaveIcons(t)
  }),
  (wave.tabsengine.showWaveIcons = function (e) {
    const t =
      waveconfig.platform == 'extension'
        ? document.body
        : document.getElementById('report').contentWindow.document
    e == 0
      ? (t.querySelectorAll('.wave5icon, .wave5text').forEach(function (e) {
          e.setAttribute('hidden', 'hidden')
        }),
        document
          .querySelectorAll('#wave5topbar .wave5icon, #wave5topbar .wave5text')
          .forEach(function (e) {
            e.setAttribute('hidden', 'hidden')
          }),
        wave.report.things.updateTopbar(),
        (wave.tabsengine.WaveIconsVisible = !1),
        wave.tabsengine.showNavOrder('show', !1))
      : (t.querySelectorAll('.wave5icon, .wave5text').forEach(function (e) {
        ...
        wave.report.things.updateTopbar(),
        (wave.tabsengine.WaveIconsVisible = !0),
        wave.tabsengine.navOrderVisible == 1 &&
          wave.tabsengine.showNavOrder('show', !0))
  }),
  (wave.tabsengine.scrollToNavIcon = function (e) {
    if (waveconfig.platform == 'extension') {
      var t = document.getElementById('waveorder' + e)
      window.scrollTo(
        0,
        t.getBoundingClientRect().top + document.documentElement.scrollTop - 30
      )
    } else {
      t = document
        .getElementById('report')
        .contentWindow.document.getElementById('waveorder' + e)
      document
        .getElementById('report')
        .contentWindow.document.documentElement.scrollTo(
          0,
      ...
      ;(a.textContent = '@keyframes blink { 50% { opacity:0 ; } }'),
        document
          .getElementById('report')
          .contentWindow.document.body.appendChild(a)
    }
    ;(t.style.animationPlayState = 'running'),
      (t.style.animation = 'blink .25s'),
      (t.style.animationIterationCount = '3'),
      (t.onanimationend = function (e) {
        ;(e.target.style.animationPlayState = 'paused'),
          (e.target.style.animation = ''),
          waveconfig.platform != 'extension' && a.remove()
      })
  }),
  (wave.tabsengine.buildNavItems = function (e) {
    let t = e.querySelectorAll(
      'a[href]:not([href=""],#wave5codetogglelink),input:not([type=hidden],[type=HIDDEN],[disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), area, [contenteditable], [tabindex]:not([class=wave5icon],[disabled]), summary'
    )
    t = Array.from(t).sort(function (e, t) {
      return (
        (e =
    ...
    n.setAttribute('id', 'waveordercontainer'),
      n.setAttribute('aria-hidden', 'true'),
      Object.assign(n.style, { position: 'absolute', top: 0, left: 0 }),
    ...
    let l = 1
    const c =
      getComputedStyle(e.body).getPropertyValue('position') == 'absolute'
        ? 380
        : 0
    return (
      t.forEach(function (e) {
        if (
          ...
          let t =
            waveconfig.platform == 'extension'
              ? window.scrollX + e.getBoundingClientRect().left - c
              : document.getElementById('report').contentWindow.scrollX +
                e.getBoundingClientRect().left -
                c
          const d =
            waveconfig.platform == 'extension'
              ? window.scrollY + e.getBoundingClientRect().top
              : document.getElementById('report').contentWindow.scrollY +
                e.getBoundingClientRect().top
          const u = document.createElement('div')
          u.classList.add('waveorder'),
            u.setAttribute('id', 'waveorder' + l),
            l++,
            ...
            (u.style.top = d + 'px'),
            n.appendChild(u)
          const g = wave.tabsengine.getAccName(e)
          i.push(g.replace(/^./, (e) => e.toUpperCase())),
            o == d &&
              r == t &&
              ((t = t + u.offsetWidth + 1), (u.style.left = t + 'px')),
            (o = d),
            (r = t),
            s++
        }
  ...
  (wave.tabsengine.getPseudoContent = function (e, t) {
    const a = window.getComputedStyle(e, t)
    const n = a.getPropertyValue('content')
    const i = a.display.substr(0, 6) === 'inline'
    return n
      ? n.substr(0, 1) !== '"'
        ? ''
        : i
  ...
  (wave.tabsengine.getAccName = function (e, t, a, n, i) {
    let s = ''
    if ((a = a || []).includes(e)) {
      if (!n) return ''
    } else a.push(e)
    if (!t && e.getAttribute('aria-labelledby')) {
      var o = e
        ...
        .map(function (e) {
          ...
          return t ? wave.tabsengine.getAccName(t, !0, a, !0) : ''
        })
      s = o.join(' ')
    }
    if (
      ...
      !s.trim() &&
        ...
        ((e.getAttribute('aria-hidden') != 'true' &&
          e.hidden != 1 &&
          getComputedStyle(e).getPropertyValue('display') != 'none' &&
          getComputedStyle(e.parentNode).getPropertyValue('display') !=
            'none') ||
          (s = '*NO ACCESSIBLE NAME*')),
      !s.trim() && t && e.tagName == 'OPTION' && (s = e.textContent),
      !s.trim() && !t && e.labels)
    )
      if (e.labels.length > 0) {
        o = Array.prototype.map.call(e.labels, function (e) {
          return wave.tabsengine.getAccName(e, !0, a)
        })
        s = o.join(' ')
      } else if (e.tagName == 'SELECT') {
        const r = e.options[e.selectedIndex] || e.querySelector('option')
        s = r
          ? e.options[e.selectedIndex] && e.options[e.selectedIndex]?.text != ''
            ? e.options[e.selectedIndex].text
            : '*NO ACCESSIBLE NAME*'
          : e.title || e.value || ''
      } else
        s =
          e.tagName === 'BUTTON' || (e.type != 'submit' && e.type != 'reset')
    ...
    if (
      ...
      const l = { FIGURE: 'figcaption', TABLE: 'caption', FIELDSET: 'legend' }
      for (const c in l)
        if (e.tagName == c) {
          const d = e.querySelector(l[c])
          d && (s = wave.tabsengine.getAccName(d, !0, a))
        }
    }
    if (
      (s.trim() ||
        e
          .querySelectorAll(
            'img:not([class=wave5icon], svg, [role=button][aria-label], [role=link][aria-label], [role=img][aria-label]'
          )
          .forEach(function (e, t) {
            if (
              e.getAttribute('aria-hidden') !== 'true' &&
      ...
      u
        .querySelectorAll(
          'style, script, noscript, svg, .wave5icon, .wave5text'
        )
        .forEach(function (e) {
          e.remove()
        }),
        (s = (s = (s = (s = u.textContent).trim().replace(/\*aria-.*?\*/, ''))
          .trim()
          .replace(/\*role=.*?\*/, ''))
          .trim()
          .replace(/\*tabindex=.*?\*/, ''))
    }
    s == '' && (s = '*NO ACCESSIBLE NAME*')
    const g = wave.tabsengine.getPseudoContent(e, ':before')
    const m = wave.tabsengine.getPseudoContent(e, ':after')
    if (t || i) return g + s + m
    let p = e.getAttribute('role')
      ? e.getAttribute('role')
      : e.tagName.toLowerCase()
    return (
      (p = (p = p == 'a' ? 'Link' : p) == 'input' ? e.type + ' ' + p : p) +
      ': ' +
      g +
      s +
      m
